...
def train_and_score(sim_id, method_name):
    work_dir = Path(f"sim{sim_id}_work")
    if not work_dir.exists():
        raise FileNotFoundError(f"Work directory {work_dir} not found. Did you run split_data.py?")
    ...
    print(f"--- Training {method_name} for Sim {sim_id} ---")
...
def main():
    ...
    try:
        train_and_score(sim_id, method_name)
