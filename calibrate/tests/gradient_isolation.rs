...
/// extreme smoothing due to numerical precision issues (see `hypothesis_fd_step_size_inconsistency`).
#[test]
fn full_gam_high_smoothing_fd_unreliable() {
    let train = create_logistic_training_data(100, 3, 31);
    let config = logistic_model_config(true, false, &train);
    let (x_gam, s_list_gam, layout, ..) =
        build_design_and_penalty_matrices(&train, &config).expect("design");
    ...
    // At rho=12, FD is unreliable so we don't assert strict agreement.
    // We only verify that both gradients are computed without error.
    assert!(cos.is_finite(), "cosine similarity should be finite");
    assert!(rel.is_finite(), "relative error should be finite");
}
