...
mod tests {
    ...
    fn test_save_load_functionality() {
        ...
        let dummy_data = TrainingData {
            y: Array::linspace(0.0, 1.0, n_samples),
            p: Array::linspace(-1.0, 1.0, n_samples),
            pcs: Array::linspace(-0.5, 0.5, n_samples).into_shape((n_samples, 1)).unwrap(),
        };
        ...
        let original_model = TrainedModel {
            config: ModelConfig {
                ...
                pc_ranges: vec![(-0.5, 0.5)],
                pc_names: vec!["PC1".to_string()],
                constraints, // Use the constraints generated by the model-building code
                knot_vectors, // Use the knot vectors generated by the model-building code
            },
            coefficients: MappedCoefficients {
                intercept: 0.5,
                main_effects: MainEffects {
                    // Use the layout to determine the correct sizes for coefficient vectors
                ...
                interaction_effects: {
                    ...
                    // The layout.pgs_main_cols gives us the range of columns for PGS main effects
                    // which tells us exactly how many PGS basis functions there are after constraints
                    let num_pgs_basis_funcs = layout.pgs_main_cols.end - layout.pgs_main_cols.start;
                    
                    // Build interaction terms
                    for i in 1..=num_pgs_basis_funcs {
                        let mut pgs_bx = HashMap::new();
                        // Each PC has exactly 9 coefficients after constraint (10 unconstrained - 1)
