    ...
    if !layout.interaction_block_idx.is_empty() {
        for (pc_idx, pc_config) in config.pc_configs.iter().enumerate() {
            ...
            if let InteractionPenaltyKind::Anisotropic = config.interaction_penalty {
                ...
                let s_pgs_raw = kronecker_product(s_pgs_penalty.as_ref(), &i_pc);
                ...
                let s_pc_raw = kronecker_product(i_pgs, s_pc_penalty.as_ref());
...
pub type EngineDims = gam::construction::EngineDims;
pub type PenaltyMatrix = gam::construction::PenaltyMatrix;
pub type PenaltyRepresentation = gam::construction::PenaltyRepresentation;
pub type ReparamInvariant = gam::construction::ReparamInvariant;
...
pub fn center_columns_in_place(x: &mut Array2<f64>, w: &Array1<f64>) {
    gam::construction::center_columns_in_place(x, w)
}

pub fn compute_penalty_square_roots(
    s_list: &[Array2<f64>],
) -> Result<Vec<Array2<f64>>, EstimationError> {
    gam::construction::compute_penalty_square_roots(s_list)
}
...
pub fn construct_s_lambda(
    ...
    gam::construction::construct_s_lambda(lambdas, s_list, p)
}
...
pub fn create_balanced_penalty_root(
    ...
    gam::construction::create_balanced_penalty_root(s_list, p)
}

pub fn kronecker_product(a: &Array2<f64>, b: &Array2<f64>) -> Array2<f64> {
    gam::construction::kronecker_product(a, b)
}
...
pub fn precompute_reparam_invariant(
    ...
    gam::construction::precompute_reparam_invariant(rs_list, p)
}
...
pub fn stable_reparameterization_engine(
    ...
    gam::construction::stable_reparameterization_engine(rs_list, lambdas, dims)
}
...
pub fn stable_reparameterization_with_invariant_engine(
    ...
    gam::construction::stable_reparameterization_with_invariant_engine(
        rs_list, lambdas, dims, invariant,
    )
}
...
pub fn stable_reparameterization(
    ...
    stable_reparameterization_engine(rs_list, lambdas, EngineDims::new(p, lambdas.len()))
}
...
mod tests {
    ...
    fn penalty_square_roots_clean_near_singular_penalty() {
        ...
        let sanitized = (&penalty + &penalty.t()) * 0.5;
    ...
    #[test]
    fn penalty_square_roots_clamp_small_negative_modes() {
        let mat = array![[1.0, 0.0], [0.0, -1e-13]];
        let roots = compute_penalty_square_roots(&[mat]).expect("square root decomposition");
        let rebuilt = roots[0].t().dot(&roots[0]);
        let (eigs, _vectors) = rebuilt.eigh(Side::Lower).expect("eigh rebuilt");
        let positive_count = eigs.iter().filter(|&&val| val > 0.0).count();
        assert!(positive_count >= 1);
        assert!(eigs.iter().all(|val| *val >= -1e-12));
        assert!(eigs.iter().any(|val| val.abs() <= 1e-12));
    }
