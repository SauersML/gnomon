...
fn main() {
    ...
    let result = match cli.command {
        ...
        Commands::Infer {
            ...
            model,
            output_path,
        } => infer_command(&test_data, &model, &output_path),
    };
...
mod tests {
    ...
    fn test_calibration_integration() -> Result<(), Box<dyn std::error::Error>> {
        ...
        println!(
            "   - Training data: {} samples ({} cases, {} controls)",
            train_rows,
        ...
        for (i, (config_name, pgs_knots, pgs_degree, pc_knots, pc_degree, penalty_order)) in
            configurations.iter().enumerate()
        {
            ...
            assert!(
                train_result.is_ok(),
                "Model training failed for config {}: {:?}",
                config_name,
                train_result
            );
            ...
            assert!(
                model_file.exists(),
                "Model file wasn't created for config {}",
                config_name
            );
            ...
            assert!(
                infer_result.is_ok(),
                "Model inference failed for config {}: {:?}",
                config_name,
                infer_result
            );
            ...
            assert!(
                ...
                "Predictions file wasn't created for config {}",
                config_name
            );
        ...
        println!(
            "   - Correlation with ground truth: {:.3} vs {:.3}",
            metrics1.correlation, metrics2.correlation
        );
        println!(
            "   - R² coefficient: {:.3} vs {:.3}",
            metrics1.r_squared, metrics2.r_squared
        );
        println!("   - AUC: {:.3} vs {:.3}", metrics1.auc, metrics2.auc);
        println!(
            "   - Brier score: {:.3} vs {:.3}",
            metrics1.brier_score, metrics2.brier_score
        );
        println!(
            "   - Mean calibration error: {:.3} vs {:.3}",
            metrics1.calibration_error, metrics2.calibration_error
        );

        // Test for consistency between models
        let consistency_correlation = calculate_correlation(&predictions1, &predictions2);
        println!(
            "\nModel consistency correlation: {:.3}",
            consistency_correlation
        );
        assert!(
            consistency_correlation > 0.9,
            "Models produce inconsistent results: correlation = {}",
            consistency_correlation
        );

        // Create a combined prediction (ensemble)
        let ensemble_predictions: Vec<f64> = predictions1
            .iter()
        ...
        let ensemble_metrics =
            validate_predictions(&ensemble_predictions, &test_data, &true_effects);
    ...
    fn test_reml_lambda_sensitivity_to_noise() -> Result<(), Box<dyn std::error::Error>> {
        ...
        // Create PGS values
        let pgs = Array1::from_vec((0..n_samples).map(|_| rng.gen_range(-2.0..2.0)).collect());
        ...
        let y = eta.mapv(|x| 1.0_f64 / (1.0 + (-x).exp())).mapv(|p| {
    ...
    fn generate_realistic_dataset() -> Result<
        (
    ...
    > {
        ...
        use rand_distr::{Beta, Gamma, Normal, Weibull};
        ...
        let cumulative_props: Vec<f64> = ancestry_clusters
            .iter()
        ...
        let generate_pc_values =
            |cluster_idx: usize, is_admixed_individual: bool, rng: &mut StdRng| -> Vec<f64> {
                ...
                if !is_admixed_individual {
                    ...
                    pcs.push(
                        Normal::new(cluster.pc1_mean, cluster.pc1_std)
                            .unwrap()
                            .sample(rng),
                    );
                    pcs.push(
                        Normal::new(cluster.pc2_mean, cluster.pc2_std)
                            .unwrap()
                            .sample(rng),
                    );
                    pcs.push(
                        Normal::new(cluster.pc3_mean, cluster.pc3_std)
                            .unwrap()
                            .sample(rng),
                    );
                    ...
                    let pc1_mean =
                        (1.0 - admix_degree) * cluster.pc1_mean + admix_degree * target.pc1_mean;
                    ...
                    let pc2_mean =
                        (1.0 - admix_degree) * cluster.pc2_mean + admix_degree * target.pc2_mean;
                    ...
                    let pc3_mean =
                        (1.0 - admix_degree) * cluster.pc3_mean + admix_degree * target.pc3_mean;
        ...
        let generate_pgs =
            |cluster_idx: usize, is_admixed_individual: bool, rng: &mut StdRng| -> f64 {
                let cluster = &ancestry_clusters[cluster_idx];
                ...
                if !is_admixed_individual {
                    ...
                    match cluster_idx {
                        0 => {
                            ...
                            let base_pgs = Normal::new(cluster.pgs_mean, cluster.pgs_std)
                                .unwrap()
                                .sample(rng);
                        ...
                        1 => {
                            ...
                            let t_component = if Uniform::new(0.0, 1.0).sample(rng) < 0.05 {
                                ...
                                2.0 * (rand::distributions::Uniform::new(-1.0_f64, 1.0_f64)
                                    .sample(rng)
                                    .signum())
                            } else {
                                0.0
                        ...
                        _ => Normal::new(cluster.pgs_mean, cluster.pgs_std)
                            .unwrap()
                            .sample(rng), // Default
                    }
                } else {
                    ...
                    let pgs_mean =
                        (1.0 - admix_degree) * cluster.pgs_mean + admix_degree * target.pgs_mean;
        ...
        fn apply_pc_correlation(pcs: &mut [f64], rng: &mut StdRng) {
            if pcs.len() < 3 {
                return;
            } // Need at least 3 PCs for correlation
        ...
        while cases.len() < desired_cases || controls.len() < desired_controls {
            ...
            let sex = if Uniform::new(0.0, 1.0).sample(&mut rng) < 0.5 {
                0.0
            } else {
                1.0
            }; // 0=female, 1=male
            ...
            let ancestry_risk = *true_effects
                .ancestry_specific_effects
            ...
            let mut eta = true_effects.intercept
                + ancestry_risk
                + true_effects.pgs_main_effect
                    * pgs_nonlinear_effect(pgs, true_effects.threshold_effects);
            ...
            for i in 0..pcs.len() {
                ...
                if let Some(effect) = true_effects.interaction_effects.get(&pc_name) {
                    ...
                    eta += effect
                        * pc_interaction_nonlinear(pcs[i], pgs, true_effects.threshold_effects);
                }
            }
            ...
            eta += Normal::new(0.0, true_effects.noise_level * noise_factor)
                .unwrap()
                .sample(&mut rng);
            ...
            let phenotype = if Uniform::new(0.0, 1.0).sample(&mut rng) < prob {
                1.0
            } else {
                0.0
            };
        ...
        let mut test_header = vec!["score".to_string(), "sex".to_string(), "age".to_string()];
    ...
    fn write_training_data_to_file(
        training_data: &(Vec<Vec<f64>>, Vec<String>),
        file_path: &str,
    ) -> Result<(), io::Error> {
    ...
    fn validate_coefficient_structure(
        model: &TrainedModel,
        true_effects: &TrueEffects,
        model_name: &str,
    ) {
        println!(
            "\n   Validating coefficient structure for {} model...",
            model_name
        );
        ...
        println!(
            "     - Strong effect avg magnitude (PC1): {:.6}",
            strong_avg_magnitude
        );
        println!(
            "     - Weak effects avg magnitude (PC3, PC4): {:.6}",
            weak_avg_magnitude
        );
        println!(
            "     - Null effects avg magnitude (PC2, PC5): {:.6}",
            null_avg_magnitude
        );
        ...
        println!(
            "     - Shrinkage threshold (relative to strong effect): < {:.6}",
            shrinkage_threshold
        );
        ...
        println!(
            "   ✓ Coefficient structure validation passed for {} model",
            model_name
        );
    }
    ...
    fn validate_smooth_function_shapes(model: &TrainedModel, true_effects: &TrueEffects) {
        ...
        if let (Some((std_pc1, _)), Some((std_pc3, _)), Some((std_pc4, _))) = (
            ...
            assert!(
                std_pc1 > std_pc3,
                "Expected PC1 effect (strong) > PC3 effect (weak)"
            );
            assert!(
                std_pc3 > std_pc4,
                "Expected PC3 effect > PC4 effect (within weak tier)"
            );
        }
    ...
    fn validate_predictions(
        ...
        let pgs_nonlinear_effect = |pgs: f64| -> f64 {
            ...
            0.4 * pgs
                + 0.3 * threshold_component
                + 0.2 * sigmoid_component
                + 0.1 * quadratic_component
        };
        ...
        for row in data {
            ...
            let mut eta =
                true_effects.intercept + true_effects.pgs_main_effect * pgs_nonlinear_effect(pgs);

            // Add PC main effects
            for i in 0..n_pcs.min(row.len() - 1) {
                // Avoid out-of-bounds
                let pc_name = format!("PC{}", i + 1);
        ...
        println!(
            "✓ Correlation between predicted and expected values: {:.3}",
            correlation
        );
        assert!(
            correlation > 0.7,
            "Correlation between predictions and expected values too low: {}",
            correlation
        );
        ...
        let mut pgs_with_predictions: Vec<(f64, f64)> = data
            .iter()
        ...
        println!(
            "✓ PGS decile means: [{:.3}, {:.3}, {:.3}, ..., {:.3}]",
            decile_means[0], decile_means[1], decile_means[2], decile_means[9]
        );
        println!(
            "✓ Monotonicity violations: {} (out of 9 transitions)",
            monotonic_violations
        );
        assert!(
            monotonic_violations <= 2,
            "Too many monotonicity violations in PGS effect: {}",
            monotonic_violations
        );

        // Also check overall trend from lowest to highest decile
        assert!(
            decile_means[9] > decile_means[0],
        ...
        println!(
            "✓ Fold increase (highest/lowest decile): {:.2}x",
            fold_increase
        );
        ...
        fn test_pc_interaction(
            ...
            let mut pc_values_with_index: Vec<(f64, usize)> = data
                .iter()
            ...
            let mut q1_pgs_values: Vec<(f64, f64)> = q1_indices
                .iter()
                .map(|&idx| (data[idx][0], predictions[idx]))
                .collect();

            let mut q4_pgs_values: Vec<(f64, f64)> = q4_indices
                .iter()
                .map(|&idx| (data[idx][0], predictions[idx]))
                .collect();
        ...
        for (i, pc_name) in (1..=n_pcs).map(|i| format!("PC{}", i)).enumerate() {
            if let Some(&interaction_coef) = true_effects.interaction_effects.get(&pc_name) {
                if interaction_coef.abs() > 0.2 {
                    // Only test significant interactions
                    ...
                    if !test_result {
                        println!(
                            "✗ Failed to detect expected {} interaction (coef: {})",
                            pc_name, interaction_coef
                        );
                        interaction_tests_passed = false;
                    } else {
                        println!(
                            "✓ Correctly detected {} interaction (coef: {})",
                            pc_name, interaction_coef
                        );
                    }
                }
        ...
        assert!(
            interaction_tests_passed,
            "Failed to detect any expected interaction effects"
        );
        ...
        let mut predictions_sorted: Vec<(f64, f64)> = predictions
            .iter()
            .zip(expected_predictions.iter())
        ...
        println!(
            "✓ Mean absolute calibration error: {:.3}",
        ...
        println!(
            "✓ Well-calibrated bins (error < 0.05): {}/10",
            well_calibrated_bins
        );
        assert!(
            well_calibrated_bins >= 5,
            "Too few well-calibrated bins: {}",
            well_calibrated_bins
        );
