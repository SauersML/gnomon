...
mod tests {
    ...
    fn eval_laml_breakdown_binom<'a>(
        ...
        let layout = DomainLayout::external(x.ncols(), rs_blocks.len());
    ...
    fn real_unpenalized_fit(
        ...
        let layout = DomainLayout::external(p, 0);
    ...
    fn double_penalty_nullspace_ridge_zero_limits() {
        ...
        let w = Array1::<f64>::ones(n);
        let layout = DomainLayout::external(x_with_ridge.ncols(), penalties_with_ridge.len());
        let cfg = ModelConfig::external(LinkFunction::Identity, 1e-3, 75, false);
        let engine_cfg = crate::calibrate::model::to_engine_model_config(&cfg)
            .expect("engine config conversion");
        let penalty_roots =
