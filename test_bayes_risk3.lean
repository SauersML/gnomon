...
theorem BayesRisk_strict_of_truth_in_closure_not_in_baseline_closure
  ...
  have h_inf_le_truth : BayesRisk R Fbig ≤ R truth := by
    ...
    intro c hc
    have h_open : IsOpen (Set.Iio c) := isOpen_Iio
    have h_preimage_open : IsOpen (R ⁻¹' (Set.Iio c)) := h_cont.isOpen_preimage (Set.Iio c) h_open
    have h_truth_mem : truth ∈ R ⁻¹' (Set.Iio c) := hc
    have h_inter_nonempty := isMemClosure_iff_nhds.mp h_truth_big (R ⁻¹' (Set.Iio c)) h_preimage_open h_truth_mem
    rcases h_inter_nonempty with ⟨y, hy_mem_fbig, hy_mem_preimage⟩
    have hy_lt_c : R y < c := hy_mem_preimage
    have h_inf_le_y : BayesRisk R Fbig ≤ R y := by
      unfold BayesRisk oracleRisk
      exact csInf_le h_bdd_big ⟨y, hy_mem_fbig, rfl⟩
