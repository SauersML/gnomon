...
$$
\text{unremovable part} = 0 \quad\Longleftrightarrow\quad \text{no pair of markers is in perfect LD}
$$

Two markers in perfect LD ($r^{2} = 1$) carry the same information. LD pruning
drops one marker from every such pair. After pruning, the unremovable part of
the error is zero. The whole error then
shrinks as the sample size and the cohort count grow.
