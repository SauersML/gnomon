...
Each part shrinks with a different kind of data. $n$ is the number of people in
the source GWAS. $d_0$ counts the parameters the source model fits. $m$ counts
the separate target cohorts. $p_i$ is how much one cohort tells you about the
transfer. $r_\perp$ is the part of the error that no data removes.
$\tilde\varepsilon^{2}$ sets the overall scale.
...
$$
n'^{*} \approx 3.29\,\ell, \qquad \mathrm{Risk}^{*} \approx \frac{16.11}{N\ell}
$$
...
$n'^{*}$ is the number of markers to genotype in one cohort. $\ell$ is the LD
decay length, measured in markers. $N$ is the total number of markers across
every cohort.

The best cohort therefore holds about 3.3 LD decay lengths of markers. The
ratio $n'^{*}/\ell$ stays between 3.27 and 3.50 across a factor of 1000 in
$\ell$, so you can use this rule in other studies.
...
different subsample of that cohort. $\tau$ is the reliability you need, between
0 and 1. The ratio $c/p$ depends on the cohort and on the trait. One
measurement fixes it, and the study below fixes it at about 88.
...
ones need about $(C/\eta)^{2k}$ samples. Here $k$ counts how indirectly the
difference appears in the data, and $C$ is a constant of the marker set. The
sample count grows without limit as $k$ grows.
...
| Best markers per cohort | $3.29\,\ell$ | agrees | across a factor of 1000 in $\ell$ |
