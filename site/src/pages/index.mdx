...
$t$ counts the generations since the two populations separated. $N_e$ is the
effective size of each population over that period.
...
through LD instead. $\rho^{2}$ is the fraction of that LD which survives in the
target population.
...
$V_E$ is the variance that the score does not explain.
...
Two markers in perfect LD ($r^{2} = 1$) carry the same information. LD pruning
