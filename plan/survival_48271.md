...
- Optimize the Fine–Gray *partial likelihood with a parametric baseline*: the Royston–Parmar spline parameterizes `log H_0`, but event contributions follow the standard Fine–Gray risk-set ratios `δ_i [η_i(a_exit) - log Σ_j R_j(a_exit) exp(η_j(a_exit))]`. Censoring/competing adjustments enter through Kaplan–Meier weights `G_j(t)` included in each risk denominator. This mirrors Beyersmann et al. (2010) and avoids conflating the parametric baseline with a full likelihood.
