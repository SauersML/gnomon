...
- Clarify formulation: we adopt the weighted Fine–Gray partial likelihood with a parametric Royston–Parmar baseline (the Beyersmann hybrid). The log-likelihood above is used only for intuition; the actual implementation keeps partial-likelihood risk-set ratios and does not build a full joint likelihood.
...
### 6.2 Event contributions and score assembly
- Event density terms still require derivative evaluations `∂f_0/∂u` and `∂g_{pgs}/∂u` (scale by `1/age`) to keep `λ_i^*(a)` positive; guard with a `1e-6` floor before logging.
- Maintain risk-set weights `s_i(t_k) = w_i G_i(t_k) exp(η_i(t_k)) / Σ_{j∈R(t_k)} w_j G_j(t_k) exp(η_j(t_k))`, with `r_i = w_i δ_i` for events. Left truncation removes individuals from `R(t_k)` until they reach `a_entry`.
- Score per individual: `U_i = r_i - Σ_{k: i∈R(t_k)} s_i(t_k)`. Cache cumulative `Σ s_i(t_k)` via prefix sums over the sorted event times to avoid repeated loops.

### 6.3 Negative Hessian assembly
- For each distinct event time `t_k`, accumulate the full risk-set cross-product `H_k = X_{R(t_k)}^⊤ [diag(s(t_k)) - s(t_k) s(t_k)^⊤] X_{R(t_k)}`. This captures the off-diagonal curvature noted in Codex feedback and keeps the Newton step stable for tied events.
- Add left-truncation effects implicitly by omitting pre-entry rows from `X_{R(t_k)}`; the diagonal of `diag(s(t_k))` stays non-negative because cumulative hazards increase with age. Apply a `1e-12` floor to denominators.
- Sum `H = Σ_k H_k` and add penalty blocks before solving.

### 6.4 Mapping to P-IRLS
- Extend `pirls::WorkingModel` so the survival family returns `(η, U, H, deviance)` directly; logistic/Gaussian variants can continue to populate diagonal-weight forms.
- Inside `pirls::run_pirls`, replace the diagonal weight shortcut with a branch: if the family provides diagonal weights, fall back to current code; otherwise solve `(H + S) Δβ = U` using the dense Faer solver. Inject a small ridge (`1e-8`) when `H` nears singular.
- Reuse the existing workspace buffers (e.g., `xtwx_buf`) for the dense Hessian and maintain symmetry during updates.
...
- Add method `TrainedModel::predict_survival_risk(age_current, horizon, pgs, pcs, sex, calibrate: bool)` returning cumulative incidence between `age_current` and `age_current + horizon` with Fine–Gray conditioning:
  ```
  let f_target = |t| 1.0 - (-H_i^*(t)).exp();
  let f_comp = cif_competing(t); // from stored Kaplan–Meier of competing events
  risk = (f_target(age_current + horizon) - f_target(age_current))
         / max(1e-12, 1.0 - f_target(age_current) - f_comp(age_current));
  ```
