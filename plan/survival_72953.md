...
- Supply derivative evaluations `∂f_{pgs×age}/∂u` alongside the smooth values so that hazard derivatives and Fine–Gray event density terms can incorporate the time-varying effect correctly during prediction and diagnostics.

### 3.4 Fine–Gray subdistribution hazard
> **Formulation note.** We intentionally adopt the weighted Fine–Gray partial likelihood with a parametric baseline cumulative
hazard, following the hybrid treatment described by Beyersmann et al. (2010). The Royston–Parmar baseline smooth supplies the
parametric piece, while the risk-set sums and score/Hessian mirror the classical Fine–Gray partial likelihood. We explicitly do
not assemble the full joint likelihood; clarifying this avoids conflating the semi-parametric weighting with the parametric
baseline spline.
...
- Because RP models provide `η_i(a_exit)` directly, we avoid time-derivative computations; however, we need `exp(η_i)` for risk-set sums and `∂ℓ/∂η` for P-IRLS. When time-varying covariate terms (e.g., PGS×age smooths) are present, retain their derivative evaluations so hazard diagnostics can include the `∂z_i(u)/∂u` contribution described in Section 3.3.
