...
- Adopt the weighted Fine–Gray partial likelihood with a Royston–Parmar baseline. This hybrid mirrors Beyersmann et al.: we keep the Fine–Gray risk-set weights and partial-likelihood ratios while the baseline spline supplies the parametric cumulative hazard. We do not attempt to write down the full joint likelihood.
...
- Follow the weighted Fine–Gray partial likelihood formulation: let `r_i = w_i d_i` for event contributions and `s_i(t_k) = w_i G_i(t_k) exp(η_i(t_k)) / risk_sum(t_k)` for each event time `t_k` where subject `i` is in the Fine–Gray risk set. The individual score is `U_i = r_i - Σ_{k: i∈R(t_k)} s_i(t_k)`, with left truncation enforced by excluding subjects from risk sets before `a_i`.
- Assemble the negative Hessian using the full risk-set cross-product for each event time: `H = Σ_k X_{R(t_k)}^⊤ [diag(s(t_k)) - s(t_k) s(t_k)^⊤] X_{R(t_k)}`. This captures the off-diagonal curvature induced by shared denominators and avoids the diagonal-weight approximation that breaks when multiple subjects fail at the same time.
- Left truncation only reduces risk-set membership prior to `a_i`; it never introduces negative working weights. After entry the cumulative hazard increment `H_i(b_i) - H_i(a_i)` is non-negative by construction, so the diagonal of each `diag(s(t_k))` block remains positive. Clamp risk sums with a `1e-12` floor for numerical stability.
- Retain derivative evaluations `∂z_i(u)/∂u` for time-varying covariates so that hazard-derivative diagnostics and any optional event-density terms stay consistent with the Royston–Parmar parameterization.

### 2.4 Absolute Risk Predictions
- Absolute risk between current age `t0` and horizon `t1` (on age scale) for covariates x:
  - Evaluate the target-cause cumulative incidence at both ages using the Fine–Gray survivor: `F_target(t) = 1 - exp(-H_i(u))` with `u = log(t)`.
  - Evaluate the competing-event cumulative incidence `F_competing(t)` from the stored Kaplan–Meier weights (or equivalently, reuse the censoring survival curve).
  - Conditional cumulative incidence between `t0` and `t1` is
    `risk = (F_target(t1) - F_target(t0)) / max(1e-12, 1 - F_target(t0) - F_competing(t0))`,
    ensuring we condition on being free of both the target and competing events at `t0`.
- Need to precompute `H_0(u)` spline basis for any requested age grid; store knots and basis degree in model config.
- For predictions over multiple horizons, evaluate basis matrix for vector of ages to avoid recomputation and share the conditional denominator across horizons for the same `t0`.
