...
- **IRLS Formulation**:
- Provide survival-specific working updates that return the score vector and the full negative Hessian assembled from risk-set cross-products `X_{R(t)}^⊤ [diag(s(t)) - s(t)s(t)^⊤] X_{R(t)}`, where `s(t)` uses the derivative-weighted hazards `w_j G_j(t) exp(η_j(t)) (∂η_j/∂a)(t) / R(t)`. This retains off-diagonal curvature induced by shared denominators.
