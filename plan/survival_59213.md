...
- Time-varying components must provide both their value and derivative with respect to `\log a` so hazard derivatives and Fine–Gray event-density terms include the `∂z_i(u)/∂u` contribution.
...
- **Primary endpoint**: Fine–Gray subdistribution hazard for competing risks.
  - Use the weighted Fine–Gray partial likelihood with a Royston–Parmar baseline (Beyersmann et al. hybrid). The partial-likelihood provides the risk-set ratios; the baseline spline supplies the parametric cumulative hazard. We deliberately avoid the Poisson trick to keep exact risk-set denominators and off-diagonal curvature.
  - Subdistribution hazard: \( \tilde{h}_k(a|X) = h_0(a) \exp( X\beta ) \).
  - Risk-set weights: maintain Kaplan–Meier censoring survival `G(a)` and form `s_i(t_k) = w_i G_i(t_k) \exp(η_i(t_k)) / \sum_{j∈R(t_k)} w_j G_j(t_k) \exp(η_j(t_k))` for each event time.
- **IRLS Formulation**:
  - Gradients follow `U_i = r_i - Σ_{k: i∈R(t_k)} s_i(t_k)` with `r_i = w_i δ_i`.
  - Assemble the negative Hessian per event as `X_{R(t_k)}^⊤ [diag(s(t_k)) - s(t_k) s(t_k)^⊤] X_{R(t_k)}`, accumulating across event times before adding penalties. This enforces the coupled risk-set structure noted by Codex and keeps the Newton step stable when multiple events occur at a shared age.
  - Left truncation simply removes individuals from `R(t_k)` before their entry age, so the diagonal of each `diag(s(t_k))` block stays non-negative (cumulative hazards increase with age). Apply a `1e-12` floor to the risk-set denominator.
  - Expose this through a custom `LinkFunction::RoystonParmar` implementation that hands the full score/Hessian to PIRLS (rather than only diagonal weights); see §5.2 for trait abstraction updates.
