...
theorem quantitative_error_of_normalization_multiplicative (k : ℕ) [Fintype (Fin k)]
  ...
  have h_risk_lower_bound :
    ...
    have h_base_int : Integrable (predictorBase m) μC := by
      ...
      -- Use h_spline_memLp
      have h_memLp := h_spline_memLp j
      have h_int : Integrable (m.pcSplineBasis.b j) (ProbabilityTheory.gaussianReal 0 1) :=
        MemLp.integrable one_le_two h_memLp
      -- Compose with coordinate projection
      have h_comp_memLp : MemLp (fun c : Fin k → ℝ => m.pcSplineBasis.b j (c l)) 2 μC := by
        have h_meas_comp : AEStronglyMeasurable (fun c => m.pcSplineBasis.b j (c l)) μC := by
          apply AEStronglyMeasurable.comp_measurable
          · exact (h_memLp).aestronglyMeasurable
          · exact measurable_pi_apply l
        rw [memLp_two_iff_integrable_sq h_meas_comp]
        rw [Measure.integral_pi_apply l]
        · exact (memLp_two_iff_integrable_sq h_memLp.aestronglyMeasurable).mp h_memLp
        · infer_instance
      exact MemLp.integrable one_le_two h_comp_memLp

    have h_base_memLp : MemLp (predictorBase m) 2 μC := by
      unfold predictorBase evalSmooth
      simp only
      apply MemLp.add (memLp_const _)
      apply MemLp.sum
      intro l _
      apply MemLp.sum
      intro j _
...
theorem prediction_is_invariant_to_affine_pc_transform_rigorous {n k p sp : ℕ} [Fintype (Fin n)] [Fintype (Fin k)] [Fintype (Fin p)] [Fintype (Fin sp)]
  ...
  let model_prime := fit p k sp n data' lambda pgsBasis splineBasis h_n_pos h_lambda_nonneg (by
    rw [Matrix.rank_eq_finrank_range_toLin, h_range_eq, ← Matrix.rank_eq_finrank_range_toLin, h_rank])
  ∀ (i : Fin n),
  ...
  have hW : LinearMap.range (Matrix.toLin' X') = W := h_range_eq.symm
  let v := X.mulVec (packParams model)
  let v' := X'.mulVec (packParams model_prime)
  have hv : Matrix.toLin' X (packParams model) ∈ W := LinearMap.mem_range_self _ _
  have hv' : Matrix.toLin' X' (packParams model_prime) ∈ W := by
    rw [← hW]
    exact LinearMap.mem_range_self _ _
  have h_loss_eq_dist : ∀ β, (1 / (n : ℝ)) * l2norm_sq (y - X.mulVec β) = (1 / (n : ℝ)) * (dist y (X.mulVec β))^2 := by
    intro β; rw [l2norm_sq, dist_eq_norm, Real.norm_eq_abs, sq_abs]; rfl
  have h_loss_eq_dist' : ∀ β', (1 / (n : ℝ)) * l2norm_sq (y - X'.mulVec β') = (1 / (n : ℝ)) * (dist y (X'.mulVec β'))^2 := by
    intro β'; rw [l2norm_sq, dist_eq_norm, Real.norm_eq_abs, sq_abs]; rfl
  have hn_inv_pos : 0 < 1 / (n : ℝ) := one_div_pos.mpr (Nat.cast_pos.mpr h_n_pos)
  ...
  have h_lin_model : linearPredictor model (data.p i) (data.c i) = v i := by
    ...
    · rfl
    · constructor <;> rfl
  have h_lin_model_prime : linearPredictor model_prime (data'.p i) (data'.c i) = v' i := by
    rw [linearPredictor_eq_designMatrix_mulVec]
    · rfl
    · constructor <;> rfl
  rw [h_lin_model, h_lin_model_prime, h_eq]
