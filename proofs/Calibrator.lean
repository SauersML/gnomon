...
theorem quantitative_error_of_normalization_multiplicative (k : ℕ) [Fintype (Fin k)]
  ...
  have h_risk_lower_bound :
    ...
    have h_decomp : ∀ m : PhenotypeInformedGAM 1 k 1, IsNormalizedScoreModel m →
        expectedSquaredError dgp (fun p c => linearPredictor m p c) =
    ...
    rw [h_decomp model_norm h_norm_opt.is_normalized, h_decomp model_star h_star_in_class]
    simp [model_star]

    have h_bias_decomp : ∫ c, (scaling_func c - model_norm.γₘ₀ 0)^2 ∂((stdNormalProdMeasure k).map Prod.snd) =
        ∫ c, (scaling_func c - 1)^2 ∂((stdNormalProdMeasure k).map Prod.snd) + (1 - model_norm.γₘ₀ 0)^2 := by
      sorry

    rw [h_bias_decomp]
    have h_base_nonneg : 0 ≤ ∫ c, (predictorBase model_norm c)^2 ∂((stdNormalProdMeasure k).map Prod.snd) := by
      apply MeasureTheory.integral_nonneg
      intro x; exact sq_nonneg _
    have h_sq_nonneg : 0 ≤ (1 - model_norm.γₘ₀ 0)^2 := sq_nonneg _
    linarith
...
noncomputable def orthogonalProjection {n : ℕ} (K : Submodule ℝ (Fin n → ℝ)) (y : Fin n → ℝ) : Fin n → ℝ :=
  ...
  let K_E := K.map (equiv.symm : (Fin n → ℝ) →ₗ[ℝ] WithLp 2 (Fin n → ℝ))
...
lemma orthogonalProjection_eq_of_dist_le {n : ℕ} (K : Submodule ℝ (Fin n → ℝ)) (y p : Fin n → ℝ)
  ...
  have h_norm_eq : ∀ v : Fin n → ℝ, l2norm_sq v = ‖equiv.symm v‖^2 := by
    intro v
    simp only [l2norm_sq, WithLp.linearEquiv_symm_apply, PiLp.norm_sq_eq_of_L2, Real.norm_eq_abs]

  have h_min_E : ∀ w_E ∈ K_E, dist (equiv.symm y) (equiv.symm p) ≤ dist (equiv.symm y) w_E := by
    intro w_E hw_E
    obtain ⟨w, hw, hw_eq⟩ := Submodule.mem_map.mp hw_E
    rw [← hw_eq]
    specialize h_min w hw
    rw [dist_eq_norm, dist_eq_norm, ← h_norm_eq, ← h_norm_eq]
    rw [LinearEquiv.map_sub, LinearEquiv.map_sub]
    exact h_min

  have h_p_E_mem : equiv.symm p ∈ K_E := Submodule.mem_map_of_mem hw_mem

  have h_proj_E := Submodule.eq_orthogonalProjection_of_mem_of_min_dist K_E (equiv.symm p) h_p_E_mem h_min_E
...
theorem prediction_is_invariant_to_affine_pc_transform_rigorous {n k p sp : ℕ} [Fintype (Fin n)] [Fintype (Fin k)] [Fintype (Fin p)] [Fintype (Fin sp)]
  ...
  -- Helper: The fit minimizes squared distance in the column space, so it is the projection
  have h_fit_is_proj : ∀ (m : PhenotypeInformedGAM p k sp) (d : RealizedData n k) (X_mat : Matrix (Fin n) (ParamIx p k sp) ℝ),
      ...
      (fun i => linearPredictor m (d.p i) (d.c i)) = orthogonalProjection (LinearMap.range (Matrix.toLin' X_mat)) d.y := by
    intro m d X_mat hX hm_in h_min

    let pred_vec := fun i => linearPredictor m (d.p i) (d.c i)
    have h_mem : pred_vec ∈ LinearMap.range (Matrix.toLin' X_mat) := by
      rw [LinearMap.mem_range]
      use packParams m
      rw [Matrix.toLin'_apply, hX]
      ext i
      rw [linearPredictor_eq_designMatrix_mulVec d pgsBasis splineBasis m hm_in]

    apply orthogonalProjection_eq_of_dist_le _ d.y pred_vec h_mem
    intro w hw
    rw [LinearMap.mem_range] at hw
    obtain ⟨beta, hbeta⟩ := hw
    let m_w := unpackParams pgsBasis splineBasis beta
    have hm_w_in : InModelClass m_w pgsBasis splineBasis := {
        basis_match := rfl, spline_match := rfl, link_identity := rfl, dist_gaussian := rfl
    }
    specialize h_min m_w hm_w_in

    have h_loss : ∀ (md : PhenotypeInformedGAM p k sp) (hmd : InModelClass md pgsBasis splineBasis),
        empiricalLoss md d lambda = (1/n) * l2norm_sq (d.y - (fun i => linearPredictor md (d.p i) (d.c i))) := by
      intro md hmd
      unfold empiricalLoss pointwiseNLL l2norm_sq
      rw [h_lambda_zero]
      simp only [add_zero, mul_zero, hmd.dist_gaussian, Pi.sub_apply]

    rw [h_loss m hm_in, h_loss m_w hm_w_in] at h_min

    -- Verify w corresponds to linearPredictor m_w
    have h_pred_w : (fun i => linearPredictor m_w (d.p i) (d.c i)) = w := by
      ext i
      rw [linearPredictor_eq_designMatrix_mulVec d pgsBasis splineBasis m_w hm_w_in]
      rw [← hX, Matrix.toLin'_apply] at hbeta
  ...
  -- Apply to model
  have hm_in : InModelClass model pgsBasis splineBasis :=
    { basis_match := rfl, spline_match := rfl, link_identity := rfl, dist_gaussian := rfl }
  have h_min_model := fit_minimizes_loss data lambda pgsBasis splineBasis h_n_pos h_lambda_nonneg h_rank

  have h_proj := h_fit_is_proj model data X rfl hm_in h_min_model

  -- Apply to model_prime
  have hm_in' : InModelClass model_prime pgsBasis splineBasis :=
    { basis_match := rfl, spline_match := rfl, link_identity := rfl, dist_gaussian := rfl }
  have h_rank' : X'.rank = Fintype.card (ParamIx p k sp) := by
    rw [← rank_eq_of_range_eq X X' h_range_eq]; exact h_rank
  have h_min_model' := fit_minimizes_loss data' lambda pgsBasis splineBasis h_n_pos h_lambda_nonneg h_rank'

  have h_proj' := h_fit_is_proj model_prime data' X' rfl hm_in' h_min_model'

  -- Conclude
  intro i
  rw [← h_proj, ← h_proj'] at i ⊢
  -- projections are equal because ranges are equal
