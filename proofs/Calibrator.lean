...
lemma orthogonalProjection_eq_of_dist_le {n : ℕ} (K : Submodule ℝ (Fin n → ℝ)) (y p : Fin n → ℝ)
  ...
  -- Use Mathlib's property in EuclideanSpace
  have h_eq_E : p_E = Submodule.orthogonalProjection K_E y_E := by
    let proj := Submodule.orthogonalProjection K_E y_E
    let proj_val : EuclideanSpace ℝ (Fin n) := proj
    have h_pythag : dist y_E p_E ^ 2 = dist y_E proj_val ^ 2 + dist proj_val p_E ^ 2 := by
      rw [dist_eq_norm, dist_eq_norm, dist_eq_norm]
      have h_orth : ⟪y_E - proj_val, proj_val - p_E⟫_ℝ = 0 := by
        exact @Submodule.orthogonalProjection_inner_eq_zero ℝ (EuclideanSpace ℝ (Fin n)) _ _ _ K_E _ y_E (proj_val - p_E) (Submodule.sub_mem K_E proj.2 h_mem_E)
      have h_decomp : y_E - p_E = (y_E - proj_val) + (proj_val - p_E) := by rw [sub_add_sub_cancel]
      rw [h_decomp]
      simp only [sq]
      rw [norm_add_sq_eq_norm_sq_add_norm_sq_real h_orth]

    have h_le : dist y_E p_E ≤ dist y_E proj_val := h_min_E proj proj.2
    have h_le_sq : dist y_E p_E ^ 2 ≤ dist y_E proj_val ^ 2 := by gcongr

    rw [h_pythag] at h_le_sq
    have h_dist_sq_le_zero : dist proj_val p_E ^ 2 ≤ 0 := by linarith
    have h_dist_eq_zero : dist proj_val p_E = 0 := by
      have h_sq_nonneg : 0 ≤ dist proj_val p_E ^ 2 := sq_nonneg _
      have h_sq_eq_zero : dist proj_val p_E ^ 2 = 0 := le_antisymm h_dist_sq_le_zero h_sq_nonneg
      exact pow_eq_zero h_sq_eq_zero
    rw [dist_eq_zero] at h_dist_eq_zero
    ext
    rw [← h_dist_eq_zero]
    rfl
