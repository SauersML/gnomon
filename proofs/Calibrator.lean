...
lemma risk_affine_additive
  ...
  -- We use a `calc` block for a more robust, step-by-step proof that is less
  -- sensitive to the exact syntactic form of the expressions than a long `rw` chain.
  calc
    ∫ pc, (pc.1 + β * pc.2 ⟨0, by norm_num⟩ - (a + b * pc.1)) ^ 2 ∂μ
    ...
    _ = a^2 + (1 - b)^2 + β^2 * (∫ pc, (pc.2 ⟨0, by norm_num⟩)^2 ∂μ) := by
      ...
      -- Apply linearity rules sequentially.
      rw [integral_sub (i_p2.add (i_c2.add (i_a2.add i_pc))) (i_p.add i_c)]
