...
theorem multiplicative_bias_correction (k : ℕ) [Fintype (Fin k)]
  ...
  have h_pred_cont_aux : ∀ (m : PhenotypeInformedGAM 1 k 1),
    ...
    · apply Continuous.add
      ...
      · apply continuous_finset_sum; intro l _; apply continuous_finset_sum; intro i _; apply Continuous.mul
        · exact continuous_const
        · rw [h_spline]; refine Continuous.comp (h_spline_cont i) ?_
    ...
    · apply continuous_finset_sum; intro idx _; apply Continuous.mul
      · apply Continuous.add
        ...
        · apply continuous_finset_sum; intro l _; apply continuous_finset_sum; intro i _; apply Continuous.mul
          ...
          · rw [h_spline]; refine Continuous.comp (h_spline_cont i) ?_
            exact (continuous_apply l).comp continuous_snd
      · rw [h_pgs]; refine Continuous.comp (h_pgs_cont _) ?_
  ...
  rcases h_capable with ⟨m, hm⟩
  have h_true_eq_m : ∀ pc : ℝ × (Fin k → ℝ), (dgpMultiplicativeBias scaling_func).trueExpectation pc.fst pc.snd = linearPredictor m pc.fst pc.snd := by
    intro pc
    exact (hm.1 pc.fst pc.snd).symm

  have h_true_cont : Continuous (fun pc : ℝ × (Fin k → ℝ) => (dgpMultiplicativeBias scaling_func).trueExpectation pc.fst pc.snd) := by
    simp_rw [h_true_eq_m]
...
theorem shrinkage_effect {p k sp : ℕ} [Fintype (Fin p)] [Fintype (Fin k)] [Fintype (Fin sp)]
  ...
  have h_eq_everywhere : ∀ p c, linearPredictor model p c = dgp_latent.to_dgp.trueExpectation p c := by
    haveI := h_measure_pos
    have h_eq_fun := (Continuous.ae_eq_iff_eq dgp_latent.to_dgp.jointMeasure h_pred_cont h_true_cont).mp h_ae_eq
    intro p c
    have h := congr_fun h_eq_fun (p, c)
    exact h
