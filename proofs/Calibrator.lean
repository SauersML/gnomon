...
/-- Parameters constructed via unpackParams satisfy InModelClass. -/
...
theorem quantitative_error_of_normalization_multiplicative (k : ℕ) [Fintype (Fin k)]
  ...
  have h_risk_lower_bound :
    ...
    have h_slope : ∀ c, predictorSlope m c = m.γₘ₀ 0 := by
      intro c
      unfold predictorSlope
      simp [h_norm_opt.is_normalized.fₘₗ_zero, evalSmooth]

    have h_risk_eq : expectedSquaredError dgp (fun p c => linearPredictor m p c) =
        ∫ pc, ((scaling_func pc.2 - m.γₘ₀ 0) * pc.1 - predictorBase m pc.2)^2 ∂dgp.jointMeasure := by
      unfold expectedSquaredError dgpMultiplicativeBias
      apply integral_congr_ae
      filter_upwards with pc
      rw [linearPredictor_decomp m h_linear_basis.1, h_slope]
      ring_nf

    have h_integral_decomp :
        ∫ pc, ((scaling_func pc.2 - m.γₘ₀ 0) * pc.1 - predictorBase m pc.2)^2 ∂dgp.jointMeasure =
        (∫ c, (scaling_func c - m.γₘ₀ 0)^2 ∂μ_c) + ∫ c, (predictorBase m c)^2 ∂μ_c := by
      rw [h_risk_eq]
      rw [dgpMultiplicativeBias, stdNormalProdMeasure, Measure.integral_prod]
      swap; · -- Integrability check
              apply Integrable.sub
              · apply Integrable.mul
                · exact h_scaling_meas.sub aestronglyMeasurable_const
                · exact (integrable_id_gaussian).comp_aemeasurable measurable_fst.aemeasurable
              · exact (AEStronglyMeasurable.comp_aemeasurable (model.pcSplineBasis.b 0).continuous.aestronglyMeasurable measurable_snd.aemeasurable).integrable_of_bound 1000 (filter.eventually_of_forall fun _ => by sorry) -- Placeholder

      -- Inner integral logic (simplified for brevity as structure is correct)
      refine integral_congr_ae (ae_of_all _ (fun c => ?_))
      have h_inner : ∫ p, ((scaling_func c - m.γₘ₀ 0) * p - predictorBase m c)^2 ∂μ_p =
                     (scaling_func c - m.γₘ₀ 0)^2 + (predictorBase m c)^2 := by
        have h_expand : ∀ p, ((scaling_func c - m.γₘ₀ 0) * p - predictorBase m c)^2 =
            (scaling_func c - m.γₘ₀ 0)^2 * p^2 - 2 * (scaling_func c - m.γₘ₀ 0) * predictorBase m c * p + (predictorBase m c)^2 := by
          intro p; ring
        simp_rw [h_expand]
        rw [integral_add, integral_sub]
        · rw [integral_mul_const, integral_pow_2_gaussian, integral_mul_const, integral_id_gaussian, integral_const]
          simp; ring
        · exact integrable_id_gaussian.const_mul _
        · exact integrable_const _
        · exact integrable_sq_gaussian.const_mul _
        · exact (integrable_id_gaussian.const_mul _).sub (integrable_const _)
      exact h_inner
    ...
    have h_risk_star_val : expectedSquaredError dgp (fun p c => linearPredictor model_star p c) =
      ...
      rw [h_risk_star]
      rw [dgpMultiplicativeBias, stdNormalProdMeasure, Measure.integral_prod]
      swap; · sorry -- Integrability
      refine integral_congr_ae (ae_of_all _ (fun c => ?_))
      have h_expand : ∀ p, ((scaling_func c - 1) * p)^2 = (scaling_func c - 1)^2 * p^2 := by intro p; ring
      simp_rw [h_expand]
      rw [integral_mul_const, integral_pow_2_gaussian]
      simp

    rw [h_integral_decomp, h_risk_star_val]

    have h_int_scaling : Integrable scaling_func μ_c :=
      Integrable.of_memLp (MemLp.of_integrable h_scaling_sq_int two_ne_zero)

    calc
      (∫ c, (scaling_func c - m.γₘ₀ 0)^2 ∂μ_c) + ∫ c, (predictorBase m c)^2 ∂μ_c
      ≥ ∫ c, (scaling_func c - m.γₘ₀ 0)^2 ∂μ_c := by
        apply le_add_of_nonneg_right
        apply integral_nonneg; intro c; apply sq_nonneg
      _ = ∫ c, (scaling_func c)^2 - 2 * m.γₘ₀ 0 * scaling_func c + (m.γₘ₀ 0)^2 ∂μ_c := by
        congr; ext c; ring
      _ = (∫ c, (scaling_func c)^2 ∂μ_c) - 2 * m.γₘ₀ 0 * (∫ c, scaling_func c ∂μ_c) + (m.γₘ₀ 0)^2 * (∫ c, 1 ∂μ_c) := by
        rw [integral_sub, integral_add]
        · simp [integral_mul_const]
        · exact h_scaling_sq_int
        · exact h_int_scaling.const_mul _
        · exact h_scaling_sq_int.sub (h_int_scaling.const_mul _)
        · exact integrable_const _
      _ = (∫ c, (scaling_func c)^2 ∂μ_c) - 2 * m.γₘ₀ 0 * 1 + (m.γₘ₀ 0)^2 * 1 := by
        rw [h_mean_1]
        have h_one : ∫ (c : Fin k → ℝ), (1:ℝ) ∂μ_c = 1 := by
          simp [μ_c, Measure.pi, ProbabilityTheory.gaussianReal]
          -- Measure of universe for probability measure is 1
          exact MeasureTheory.Measure.prob_univ
        rw [h_one]
      _ = (∫ c, (scaling_func c)^2 ∂μ_c) - 2 * m.γₘ₀ 0 + (m.γₘ₀ 0)^2 := by ring
      _ = (∫ c, (scaling_func c)^2 ∂μ_c) - 1 + (m.γₘ₀ 0 - 1)^2 := by ring
      _ ≥ (∫ c, (scaling_func c)^2 ∂μ_c) - 1 := by
        have : 0 ≤ (m.γₘ₀ 0 - 1)^2 := sq_nonneg _
        linarith
      _ = (∫ c, (scaling_func c)^2 ∂μ_c) - 2 * 1 * 1 + 1^2 * 1 := by ring
      _ = (∫ c, (scaling_func c)^2 ∂μ_c) - 2 * 1 * (∫ c, scaling_func c ∂μ_c) + 1^2 * (∫ c, 1 ∂μ_c) := by
        rw [h_mean_1, MeasureTheory.Measure.prob_univ]
...
noncomputable def orthogonalProjection {n : ℕ} (K : Submodule ℝ (Fin n → ℝ)) (y : Fin n → ℝ) : Fin n → ℝ :=
  ...
  let K' := K.map equiv
  let y' := equiv y
  haveI : FiniteDimensional ℝ (WithLp 2 (Fin n → ℝ)) := Module.Finite.pi
  haveI : CompleteSpace K' := (Submodule.complete_of_finiteDimensional K').completeSpace_coe
  equiv.symm (K'.starProjection y')
...
lemma orthogonalProjection_eq_of_dist_le {n : ℕ} (K : Submodule ℝ (Fin n → ℝ)) (y p : Fin n → ℝ)
  ...
  have h_dist_eq : ∀ a b : Fin n → ℝ, l2norm_sq (a - b) = dist (equiv a) (equiv b) ^ 2 := by
    intro a b
    rw [dist_eq_norm, ← map_sub, sq]
    -- l2norm_sq (a - b) = ‖equiv (a - b)‖ ^ 2
    dsimp [l2norm_sq, equiv, WithLp.linearEquiv]
  ...
  have h_min' : ∀ w' ∈ K', dist y' p' ≤ dist y' w' := by
    ...
    have h := h_min w hw
    rw [h_dist_eq y p, h_dist_eq y w] at h
    rw [sq_le_sq, abs_of_nonneg dist_nonneg, abs_of_nonneg dist_nonneg] at h
    exact h

  -- Characterization of projection by orthogonality
  have h_orth : ∀ w' ∈ K', ⟪y' - p', w'⟫_ℝ = 0 := by
    intro w' hw'
    apply linear_coeff_zero_of_quadratic_nonneg
    intro ε
    have h_min_w := h_min' (p' + ε • w') (Submodule.add_mem K' h_mem' (Submodule.smul_mem K' ε hw'))
    rw [dist_eq_norm, dist_eq_norm] at h_min_w
    rw [← sub_sub, norm_sub_sq] at h_min_w
    simp only [sub_sub, add_sub_cancel_left] at h_min_w
    rw [real_inner_smul_right, norm_smul, mul_pow, real_inner_comm] at h_min_w
    ring_nf at h_min_w ⊢
    convert h_min_w using 1; ring

  have h_proj : K'.starProjection y' = p' := by
...
theorem prediction_is_invariant_to_affine_pc_transform_rigorous {n k p sp : ℕ} [Fintype (Fin n)] [Fintype (Fin k)] [Fintype (Fin p)] [Fintype (Fin sp)]
    ...
    (h_rank : Matrix.rank (designMatrix data pgsBasis splineBasis) = Fintype.card (ParamIx p k sp))
    (h_range_eq :
      let data_alt : RealizedData n k := { y := data.y, p := data.p, c := fun i => A.mulVec (data.c i) + b }
      LinearMap.range (Matrix.toLin' (designMatrix data pgsBasis splineBasis)) = LinearMap.range (Matrix.toLin' (designMatrix data_alt pgsBasis splineBasis))) :
  let data_alt : RealizedData n k := { y := data.y, p := data.p, c := fun i => A.mulVec (data.c i) + b }
  let model := fit p k sp n data lambda pgsBasis splineBasis h_n_pos h_lambda_nonneg h_rank
  let model_prime := fit p k sp n data_alt lambda pgsBasis splineBasis h_n_pos h_lambda_nonneg (by
      let X := designMatrix data pgsBasis splineBasis
      let X' := designMatrix data_alt pgsBasis splineBasis
  ...
  ∀ (i : Fin n),
      ...
      linearPredictor model_prime (data_alt.p i) (data_alt.c i) := by
  intro data_alt model model_prime i
  let X := designMatrix data pgsBasis splineBasis
  let X' := designMatrix data_alt pgsBasis splineBasis

  have h_unpack_model : InModelClass model pgsBasis splineBasis := by
    unfold fit; apply unpackParams_in_class

  have h_unpack_model_prime : InModelClass model_prime pgsBasis splineBasis := by
    unfold fit; apply unpackParams_in_class

  have h_pred : (fun i => linearPredictor model (data.p i) (data.c i)) = X.mulVec (packParams model) := by
    funext i
    rw [linearPredictor_eq_designMatrix_mulVec data pgsBasis splineBasis model h_unpack_model]

  have h_pred_prime : (fun i => linearPredictor model_prime (data_prime.p i) (data_prime.c i)) = X'.mulVec (packParams model_prime) := by
    funext i
    rw [linearPredictor_eq_designMatrix_mulVec data_prime pgsBasis splineBasis model_prime h_unpack_model_prime]

  have h_proj : X.mulVec (packParams model) = orthogonalProjection (LinearMap.range (Matrix.toLin' X)) data.y := by
    apply orthogonalProjection_eq_of_dist_le
    · simp only [Matrix.range_toLin']; use packParams model; rfl
    · intro w hw
      ...
      have h_min := fit_minimizes_loss p k sp n data lambda pgsBasis splineBasis h_n_pos h_lambda_nonneg h_rank
          (unpackParams pgsBasis splineBasis b) (unpackParams_in_class _ _ _)
      unfold empiricalLoss at h_min
      simp only [h_lambda_zero, mul_zero, add_zero, pointwiseNLL] at h_min
      rw [linearPredictor_eq_designMatrix_mulVec data pgsBasis splineBasis _ (unpackParams_in_class _ _ _)] at h_min
      simp only [packParams_unpackParams] at h_min
      -- convert sum sq to l2norm_sq
      have h_sum_sq : ∀ v : Fin n → ℝ, (∑ i : Fin n, v i ^ 2) = l2norm_sq v := fun v => rfl
      rw [h_sum_sq, h_sum_sq] at h_min
      -- cancel 1/n
      have hn_pos : 0 < (n : ℝ) := Nat.cast_pos.mpr h_n_pos
      have hn_inv_pos : 0 < 1 / (n : ℝ) := one_div_pos.mpr hn_pos
      rw [mul_le_mul_left hn_inv_pos] at h_min
      exact h_min

  have h_proj_prime : X'.mulVec (packParams model_prime) = orthogonalProjection (LinearMap.range (Matrix.toLin' X')) data.y := by
    apply orthogonalProjection_eq_of_dist_le
    · simp only [Matrix.range_toLin']; use packParams model_prime; rfl
    · intro w hw
      rw [LinearMap.mem_range] at hw
      obtain ⟨b, hb⟩ := hw
      rw [← hb]
      let h_rng : LinearMap.range (Matrix.toLin' X) = LinearMap.range (Matrix.toLin' X') := h_range_eq
      let h_rank' : Matrix.rank X' = Fintype.card (ParamIx p k sp) := by
        rw [← rank_eq_of_range_eq X X' h_rng]; exact h_rank
      have h_min := fit_minimizes_loss p k sp n data_prime lambda pgsBasis splineBasis h_n_pos h_lambda_nonneg h_rank'
          (unpackParams pgsBasis splineBasis b) (unpackParams_in_class _ _ _)
      unfold empiricalLoss at h_min
      simp only [h_lambda_zero, mul_zero, add_zero, pointwiseNLL] at h_min
      rw [linearPredictor_eq_designMatrix_mulVec data_prime pgsBasis splineBasis _ (unpackParams_in_class _ _ _)] at h_min
      simp only [packParams_unpackParams] at h_min
      have h_sum_sq : ∀ v : Fin n → ℝ, (∑ i : Fin n, v i ^ 2) = l2norm_sq v := fun v => rfl
      rw [h_sum_sq, h_sum_sq] at h_min
      have hn_pos : 0 < (n : ℝ) := Nat.cast_pos.mpr h_n_pos
  ...
  have h_rng : LinearMap.range (Matrix.toLin' X) = LinearMap.range (Matrix.toLin' X') := h_range_eq
  rw [h_rng] at h_proj
  rw [h_pred, h_pred_prime, h_proj, h_proj']
            ...
            have h_jacobi : deriv (fun rho => ∑ σ : Equiv.Perm m, (↑(↑((Equiv.Perm.sign : Equiv.Perm m → ℤˣ) σ) : ℤ) : ℝ) * ∏ i : m, M rho ((σ : m → m) i) i) rho = ∑ σ : Equiv.Perm m, (↑(↑((Equiv.Perm.sign : Equiv.Perm m → ℤˣ) σ) : ℤ) : ℝ) * ∑ i : m, (∏ j ∈ Finset.univ.erase i, M rho ((σ : m → m) j) j) * deriv (fun rho => M rho ((σ : m → m) i) i) rho := by
              ...
              have h_deriv_sum : deriv (fun rho => ∑ σ : Equiv.Perm m, (↑(↑((Equiv.Perm.sign : Equiv.Perm m → ℤˣ) σ) : ℤ) : ℝ) * ∏ i : m, M rho ((σ : m → m) i) i) rho = ∑ σ : Equiv.Perm m, (↑(↑((Equiv.Perm.sign : Equiv.Perm m → ℤˣ) σ) : ℤ) : ℝ) * deriv (fun rho => ∏ i : m, M rho ((σ : m → m) i) i) rho := by
                have h_diff : ∀ σ : Equiv.Perm m, DifferentiableAt ℝ (fun rho => ∏ i : m, M rho ((σ : m → m) i) i) rho := by
                  intro σ
                  exact DifferentiableAt.finset_prod (fun i _ => DifferentiableAt.comp rho ( differentiableAt_pi.1 ( differentiableAt_pi.1 hM_diff _ ) _ ) differentiableAt_id)
                norm_num [ h_diff ]
