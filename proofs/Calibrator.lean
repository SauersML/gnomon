...
noncomputable def orthogonalProjection {n : ℕ} (K : Submodule ℝ (Fin n → ℝ)) (y : Fin n → ℝ) : Fin n → ℝ :=
  ...
  equiv (proj_E : EuclideanSpace ℝ (Fin n))
...
lemma orthogonalProjection_eq_of_dist_le {n : ℕ} (K : Submodule ℝ (Fin n → ℝ)) (y p : Fin n → ℝ)
  ...
  have h_min_E : ∀ w_E ∈ K_E, ‖y_E - p_E‖ ≤ ‖y_E - w_E‖ := by
    ...
    have h_norm_sq : ∀ v, l2norm_sq v = ‖equiv.symm v‖^2 := by
      intro v
      simp only [l2norm_sq, PiLp.norm_eq_of_L2, Real.norm_eq_abs, sq_abs]
      dsimp [equiv]
      rw [Real.sq_sqrt (Finset.sum_nonneg fun _ _ => sq_nonneg _)]
    rw [h_norm_sq, h_norm_sq] at h_le
    dsimp [y_E, p_E]
    rw [← map_sub, ← map_sub]
    rw [sq_le_sq, abs_of_nonneg (norm_nonneg _), abs_of_nonneg (norm_nonneg _)] at h_le
    exact h_le

  have h_orth : y_E - p_E ∈ K_E.orthogonal := by
    rw [Submodule.mem_orthogonal]
    intro v_E hv_E
    let f : ℝ → ℝ := fun t => ‖y_E - (p_E + t • v_E)‖^2
    have h_min_t : ∀ t, f 0 ≤ f t := by
      intro t
      have h_mem_t : p_E + t • v_E ∈ K_E := Submodule.add_mem K_E h_mem_E (Submodule.smul_mem K_E t hv_E)
      have h_le_sq := h_min_E _ h_mem_t
      simp only [f]
      simp only [zero_smul, add_zero]
      rw [sq_le_sq, abs_of_nonneg (norm_nonneg _), abs_of_nonneg (norm_nonneg _)]
      exact h_le_sq

    have h_expand : ∀ t, f t = ‖y_E - p_E‖^2 - 2 * t * ⟪y_E - p_E, v_E⟫_ℝ + t^2 * ‖v_E‖^2 := by
      intro t
      simp only [f]
      rw [sub_add_eq_sub_sub]
      rw [norm_sub_sq_real]
      simp only [inner_smul_right, norm_smul, Real.norm_eq_abs, mul_pow, sq_abs, real_inner_comm]
      ring

    have h_quad : ∀ t, (-2 * ⟪y_E - p_E, v_E⟫_ℝ) * t + ‖v_E‖^2 * t^2 ≥ 0 := by
      intro t
      specialize h_min_t t
      rw [h_expand t, h_expand 0] at h_min_t
      simp only [sub_zero, zero_mul, add_zero, mul_zero] at h_min_t
      linarith

    have h_coeff_zero := linear_coeff_zero_of_quadratic_nonneg (-2 * ⟪y_E - p_E, v_E⟫_ℝ) (‖v_E‖^2) h_quad
    rw [real_inner_comm] at h_coeff_zero
    linarith

  let P_y := Submodule.orthogonalProjection K_E y_E
  have h_orth_P : y_E - (P_y : EuclideanSpace ℝ (Fin n)) ∈ K_E.orthogonal := Submodule.sub_orthogonalProjection_mem_orthogonal y_E
  have h_mem_P : (P_y : EuclideanSpace ℝ (Fin n)) ∈ K_E := P_y.2

  have h_diff_mem : (P_y : EuclideanSpace ℝ (Fin n)) - p_E ∈ K_E := Submodule.sub_mem K_E h_mem_P h_mem_E
  have h_diff_orth : (P_y : EuclideanSpace ℝ (Fin n)) - p_E ∈ K_E.orthogonal := by
    have h_eq : (P_y : EuclideanSpace ℝ (Fin n)) - p_E = (y_E - p_E) - (y_E - P_y) := by abel
    rw [h_eq]
    apply Submodule.sub_mem K_E.orthogonal
    · exact h_orth
    · exact h_orth_P
  ...
  have h_eq_0 : (P_y : EuclideanSpace ℝ (Fin n)) - p_E = 0 := by
    ...
    exact ⟨h_diff_mem, h_diff_orth⟩

  dsimp [orthogonalProjection]
  rw [← equiv.apply_symm_apply p]
  apply equiv.injective
  exact (eq_of_sub_eq_zero h_eq_0).symm
...
theorem prediction_is_invariant_to_affine_pc_transform_rigorous {n k p sp : ℕ} [Fintype (Fin n)] [Fintype (Fin k)] [Fintype (Fin p)] [Fintype (Fin sp)]
  ...
  let model_prime : PhenotypeInformedGAM p k sp := sorry
  ∀ (i : Fin n),
