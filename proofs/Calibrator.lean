...
theorem prediction_is_invariant_to_affine_pc_transform_rigorous_proved {n k p sp : ℕ}
    ...
    (h_data' : data' = { y := data.y, p := data.p, c := fun i => A.mulVec (data.c i) + b })
  ...
  have h_y_eq : data.y = data'.y := by rw [h_data']
...
theorem context_specificity_proved {p k sp : ℕ} [Fintype (Fin p)] [Fintype (Fin k)] [Fintype (Fin sp)] (dgp1 dgp2 : DGPWithEnvironment k)
    ...
    (h_realizable2_impl : IsBayesOptimalInClass dgp2.to_dgp model1 → ∀ p c, linearPredictor model1 p c = dgp2.to_dgp.trueExpectation p c) :
  ¬ IsBayesOptimalInClass dgp2.to_dgp model1 := by
  intro h_opt2
  have h_realizable2 := h_realizable2_impl h_opt2
  ...
  have h_eq : dgp1.to_dgp.trueExpectation = dgp2.to_dgp.trueExpectation := by
    funext p c
    rw [← h_realizable1 p c, h_realizable2 p c]
  exact h_neq h_eq
