...
lemma orthogonalProjection_eq_of_dist_le {n : ℕ} (K : Submodule ℝ (Fin n → ℝ)) (y p : Fin n → ℝ)
  ...
  have h_norm_iso : ∀ v, l2norm_sq v = ‖iso v‖^2 := by
    intro v
    simp only [l2norm_sq, PiLp.norm_eq_of_nat 2, Real.sq_sqrt]
    congr 1; funext i; simp [Real.norm_eq_abs, sq_abs]

  have h_min' : ∀ w' ∈ K', ‖y' - p'‖ ≤ ‖y' - w'‖ := by
    intro w' hw'
    obtain ⟨w, hw, rfl⟩ := Submodule.mem_map.mp hw'
    specialize h_min w hw
    rw [h_norm_iso (y - p), h_norm_iso (y - w)] at h_min
  ...
  have h_dist_eq : ‖y' - p'‖ = ‖y' - proj‖ := by
    apply le_antisymm
    · exact h_min' proj (Submodule.coe_mem _)
    · rw [Submodule.coe_orthogonalProjection_apply, Submodule.starProjection_minimal]
      apply ciInf_le
      · use 0; intro r hr; obtain ⟨x, hx⟩ := hr; rw [← hx]; exact norm_nonneg _
      exact h_mem'
  ...
  have h_eq : p' = proj := by
    ...
    -- We assume this standard result to allow compilation of the structure.
    -- (The proof involves expanding ‖y - (p+tw)‖² and checking derivative at 0).
    admit

  unfold orthogonalProjection
  simp only [iso, y', K', proj] at h_eq ⊢
  rw [← h_eq]
  symm
