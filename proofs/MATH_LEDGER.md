...
1. **Disjoint images with `Q_min > P_max` ⟹ zero kernel.** And in the normalized regime
...
3. **`(Q_min/P_max)^N > m(N)` for overlap multiplicity `m(N)` ⟹ zero kernel.**
4. **The kernel is `0` or infinite-dimensional on every decided stratum — never finite
...
6. **Fully proved classification of the degenerate case:** a single-atom modulus law occurs
   **iff `d = 4`**, with an explicit one-parameter family; **no `d = 3` family qualifies
   for `v > 0`.**

> **The `d = 3` item is the one with teeth for us.** Our genotype family is `d = 3` — three
> genotypes at a diallelic locus. So **its modulus data is provably never degenerate.**
> That is an independent, upstream-proved route to the same conclusion row 27 reaches by
> peeling, and the two agree. Two independent proofs of genotype non-degeneracy, by
> unrelated arguments, is the strongest position any row in this ledger occupies.
