...
**CITE THE RUN, NOT ONLY THE VERDICT.** A SIMULATED figure should name the script and,
where it matters, the revision or result file it came from. This is not bookkeeping. On
the ledger's first refresh, row 67 imported a `14σ` anomaly that had been **retracted
hours earlier**: the `± 0.05` travelled attached to the number and made it look precise,
and the retraction did not travel with it at all. A superseded figure should be
detectable by looking up its run, not by someone happening to remember that it was
withdrawn.

**And prefer recording a retraction to deleting it.** Row 67 keeps the withdrawn `14σ`
together with the mechanism that dissolved it — 9 of 27 rows sharing one measurement
while the fit treated them as independent. "An anomaly that was withdrawn, and why" is
more useful to the next reader than a clean row, because the clean row invites the same
measurement to be made again.
...
**Marker census, and how to reproduce it in five seconds.** Do not trust the numbers
below; run the command. They were hand-maintained until 2026-08-02 and had drifted from
402/323/~20/~8 to 445/320/30/11 while still *reading* as current, because the `UNTESTED`
count had not moved and one matching number makes four look checked. A summary statistic
that cannot report its own staleness is the thing this ledger exists to catch.

```
git grep -oh "Empirical status: [A-Z][A-Z ]*" -- 'Calibrator*' \
  | sed 's/ *$//' | sort | uniq -c | sort -rn
```

As of 2026-08-02, 445 markers total: 320 `UNTESTED`, 58 `DERIVED`, 30 `VALIDATED`,
11 `FALSIFIED`, 5 `CONDITIONALLY VALID`, 3 `VACUOUS`, and 7 one-off statuses
(`TESTED`, `MEASURED`, `NUMERATOR VALIDATED`, `FORMULA CORRECTED AND MEASURED`,
`CONVENTION PINNED`, `CONVENTION IDENTIFIED`). Row count: `grep -cE '^\| [0-9]+ \|'`.
...
#### ROW 23 IS NOW STALE — the misnomer was REPAIRED and this block was not updated

**Read this before the block below.** An audit on 2026-08-03 enumerated every `hudsonFst`
site and found the defect **fixed**: `Conventions.lean:246` now computes genuine Hudson
`(p₁-p₂)²/(p₁(1-p₂)+p₂(1-p₁))` with the Bhatia et al. citation, `hudsonFst_eq_of_neiGst`
carries `H = 2G/(1+G)`, the Nei side is separately named throughout (`neiGst`,
`neiContrastSpike`, `four_neiGst_eq_standardizedContrastVariance`), and
`neiGstFromFrequencies_eq_hudsonFst` was renamed `..._eq_neiGst`. **All eleven dependent
sites are correct**, including the four in `DemographicCapacity` this block says inherit the
misnomer. `map/correctability.rs:170-173` validates its input as "Hudson F_ST scale" and
`analyze.py`'s stale `KAPPA = 2` is gone.

So three specific claims below are false of the current tree: that the name is uncorrected on
the Nei body, that `DemographicCapacity`:50 and :67–136 inherit it, and that `analyze.py`
documents `KAPPA = 2`. **The staleness runs in the dangerous direction** — a reader acting on
this block would "fix" a defect that is already fixed, and could reintroduce it. The
historical account below is retained because the reasoning is the precedent the corpus uses
for the whole misnomer class, but it is history, not a work item.
...
upstream `hudsonFst` conversion `Hudson = 2G/(1+G)` reproduces the measured ratio
...
### The freezing transition, and the effective-independence dimension (rows 52–53)

| # | RESULT | STATUS | WHERE IN CORPUS | BIOLOGY | EVIDENCE | GAP |
|---|---|---|---|---|---|---|
| 52 | **The freezing transition.** Residual discreteness `(1 − 1/ℓ)^(n−1)`. | measured and confirmed | **NOT WIRED** | **Operational form, and this is the part to carry: the number of effectively independent BLOCKS — not the number of markers — decides when a normal approximation is safe.** That is a directly actionable statement about when the Gaussian score assumption may be used, and it is the kind of claim `ScoreDistribution`'s Berry–Esseen certificates are built to consume. | **MEASURED, with the predicted failure also failing as predicted.** Max deviation `0.033` against a binomial standard error of `0.024`. The negative arm — a **non-renewal chain**, where the law predicts the approximation should break — **did break**. | **The two-sided design is why this row is strong.** A confirmation whose matching prediction-of-failure also fires is a test that was capable of failing and did not, which is the standard row 47 now imposes on every negative claim. Wiring target: this is the natural bridge from the arc to `ScoreDistribution`, which is one of the few biology modules the arc already *imports* (see the wiring section) — so the import direction is favourable for once. |
| 53 | **The effective-independence dimension `D`.** Unifies three previously separate results and **merges two thresholds into one**: local theory applies **iff `min(panel dimension, D) > order log N`**. | standing | **NOT WIRED** | A single criterion replacing two, with `D` the quantity that decides. | ASSERTED here (I have the statement, not the derivation). | **This is the best consolidation target in the ledger.** Three results collapsing to one criterion is the shape that reduces surface area rather than adding to it — the opposite of most rows here. It also connects to row 52: "effectively independent blocks" and "effective-independence dimension" are plausibly the same quantity wearing two names, and if they are, rows 52 and 53 should be merged before either is formalized. **Worth checking before anyone builds on either.** |
...
confirmation*. **The new `hudsonFst` conversion supplies it analytically**, and it
...
### Rows 56–58: the coverage boundary, the block count, and long-memory geometry

> **BUILD STATUS: COMPILED on acn112, 2026-08-03**, modules green. **VALIDATION STATUS:
> REPORTED for rows 55, 56 and 57 — and it refuted a claim in each of the three.** See the
> outcome block after row 58. Remaining arms (certificate grading, long memory, ridge, moment
> body, width law) still in flight.
>
> *(superseded wording, preserved: "VALIDATION STATUS: in flight"* — three simulation arms were dispatched for rows 56 and 57 and for the row-55
> error floor; no row below may be cited as measured until those report. Where a row says
> PROVED it means the algebra, never the analytic input above it.

| # | RESULT | STATUS UPSTREAM | WHERE IN CORPUS | BIOLOGY | EVIDENCE | GAP |
|---|---|---|---|---|---|---|
| 56 | **The coverage-invariance boundary, witnessed.** Coverage of modulus cells is invariant under couplings of arbitrary strength while every joint cell keeps positive mass, and fails exactly at `η = 0`. | standing (Part XI, principle P5) | `ConditionalGain.lean` §boundary: `copyWitnessFamily`, `modulusCopyCoupling`, `modulusCopyCoupling_not_fullSupport`, `copyWitness_productCovers`, `copyWitness_not_coversTuple`, `coverage_invariance_sharp` | **Identifiability survives arbitrary LD and dies only at perfect LD.** At a diallelic pair `η = 0` is exactly `r² = 1`, so pruning at `r² = 1` removes the whole obstruction and pruning below it removes nothing coverage depended on. | **PROVED**, exact and finite | **This row exists because `coverage_invariant` had a hypothesis never shown capable of failing.** Its clause "until it kills support" was doing all the work with no witness behind it — precisely the vacuity pattern rows 43 and 47 track, caught in our own work this time. The remaining gap is the **population-versus-sample** one: the theorem is about positive mass, and a cell of positive but tiny probability is empirically unobserved at finite `n`. That boundary is the one a study actually meets and it is not what this theorem addresses. |
| 57 | **The Berry–Esseen count is blocks, not markers.** A marker-counted normal-approximation bound understates the true one by exactly `√ℓ` at correlation length `ℓ`. | standing (Part VIII, freezing transition) | `ScoreDistribution.lean` §BlockCount: `effectiveBlockCount`, `residualDiscreteness`, `berry_esseen_block_bound_eq`, `marker_count_understates_berry_esseen` | **A correction to a live theorem in this corpus, not an addition.** `berry_esseen_error_decreases_with_snps` counts markers; under linkage that is wrong in the **anti-conservative** direction, so a bound that is too small reads as "the Gaussian approximation is safe here". The error is worst exactly where LD is strongest and the marker count most flattering. A million variants at correlation length 100 have the normal-approximation quality of ten thousand. | Algebra **PROVED**. The block reduction itself is an **analytic input** (regeneration / excursion decomposition, `FoldedSpectrum` §8) and is carried as one, not derived. | **The load-bearing step is the untested one**, and it is the assumption rather than the algebra: that a linked score obeys a CLT with the block count in place of the marker count. Validation dispatched, including a non-renewal negative arm where the reduction should fail. Until it reports, this row is a conditional. |
| 57v | **VALIDATION OUTCOME on row 57 — the count is confirmed, the constant is FALSIFIED.** | measured | `ScoreDistribution` §BlockCount, rewritten | The block *count* is right: ratio `1.00 ± 0.01` for block-constant haplotypes at every `ℓ ∈ [1,32]`, `m ∈ [16,8192]`. The *constant* is not: for geometric block lengths — a genuine renewal chain — the ratio is **2.09** at `ℓ = 32`, 25–240 SE from one, and the corpus's worked example was off by 2× **anti-conservatively**, the exact failure it warns about. | **MEASURED**, 153 configs, 1.6M individuals each, positive control (`ℓ=1`, three independent generators) reproducing the analytic value to `1.001 ± 0.003`. Closed form `κ = E[bw³]/E[bw²]^{3/2} → 2.1213`; predicted vs measured under 2% at every `ℓ`. Heterogeneous effects give `κ → 0.6267` (measured `0.65`); both together `1.319` vs `1.31`. | `κ` is now an explicit argument and `marker_count_understates_berry_esseen` carries `1 < κ√ℓ`, because the sign goes both ways. **Three further defects recorded in-file:** the `FoldedSpectrum` §8 citation licenses an excursion-bundle panel, not `m/ℓ` markers, and that collapse *is* the `κ=1` smuggle; `ℓ` has no operational definition and its estimators disagree by `√2` or return `1` outright; and for a lattice score the bound and the measured KS distance move in **opposite** directions. |
| 55v | **VALIDATION OUTCOME on row 55 — the abstract bound holds, the instantiation is VACUOUS at native scale.** | measured | `CountingInvariantInstances`, header rewritten | Measured `L*` (tightest constant consistent with the witness, hence maximally generous): Li–Ji `1024.9`, participation ratio `993.9`, Galwey `720.3`, against `n = 32` — **floor negative for three of the four formulas in use**, and `L* ~ n²` so it diverges. Cheverud–Nyholt survives but is empty: floor `0.471` against error `1055`. | **MEASURED**, exact rationals, `n ≤ 32`, moments to order 8. Positive control passes: the script re-derives `certGap = n/(n+1)` exactly and every order-2 moment gap within `1/(n+1)`. | Content is real only in **normalized** units (`m_eff/m`), where error/floor `= 2.000` and the 2 is structural. The Lipschitz hypothesis was global — satisfied by no real formula — and is now localized to the two witness points. Two scope errors recorded: Galwey uses a half-integer moment and is not in the class; and **Li–Ji equals `m` exactly** on any trace-legal spectrum with eigenvalues below 2. Upstream defect found: `meffWitness_spectrum_pos`'s "spectra a correlation matrix can have" is false (trace ≠ `m`), and the "uniformly in the order `p`" claim is an artifact of that illegal normalization — it holds only to order 2. |
| 56v | **VALIDATION OUTCOME on row 56 — the population claim survives, the `r²` framing does not.** | measured | `ConditionalGain` §boundary, rewritten twice | Coverage is bit-identical to the linkage-equilibrium baseline at **every** `r² < 1`, down to a joint cell mass of `6.25e-23` at `r² = 1 - 10⁻⁹`, in exact rationals. But of 72 configurations with a vanishing haplotype at `r² < 1`, **56 lose coverage strictly, the worst at `r² = 1/81 ≈ 0.0123`** — so no `r²` threshold certifies the positive-floor hypothesis. | **MEASURED**, exact rational sweep plus 200-replicate finite-sample arm. | **Linkage is not even the dominant driver.** At `p = 1/20` with `r² = 0` exactly, `N = 500k` loses a cell in 5% of runs — allele frequency alone, no LD. And raising `r²` from `0` to `0.81` there *increases* the rarest cell probability (`6.25e-6 → 2.26e-5`): **more LD made coverage easier.** Any rule phrased in `r²` gets both backwards. |
| 54v | **VALIDATION OUTCOME on row 54 — part (III) FAILED its first measurement; parts (I), (II), (IV) untouched.** | measured | `CertificateGrading.lean`, header rewritten; `gradeSound_of_nested_sup` added | **The headline gap collapses.** Measured deficit `Δ₀/Δ₈` = `3.284, 1.424, 1.007, 1.012, 1.027` at `h = 1e-1 … 1e-6`, reaching `1.0007` at `h = 1e-9` on a growing support: **a grade-8 certificate recovers 99.93% of the ungraded modulus.** By `isComplete_iff_gradeInsensitive` every fixed grade is then very nearly *complete* — the opposite of (III). `b_order` falsified in-instance (fitted exponent grade-independent to 0.7%, so `b_K·K` grows linearly). Envelope fitted `1/log(1/h)^{0.966}` at fixed support, not `1/√log(1/h)`. | **MEASURED** by exact LP (HiGHS), Gaussian location mixture, `F = E|θ|`, grid refinement moving answers <0.1%. Best-approximation duality control PASSES to 5 digits with exact anchors `E₀=E₁=0.5`, `E₂=E₃=0.125`. | **The mechanism is the finding, not the numbers.** At small `h` the grade-0 optimiser already matches moments to order 4 unbidden (`2.1e-11, 3.0e-9, 2.1e-9, 2.0e-7` at `h=1e-9`), so the grading constraint goes vacuous exactly in the regime the theory is about. **And a conceptual objection outlives the numbers:** grading by moment count makes higher grades weaker, which is right for the definition but does *not* order the four named methods by power — moment-matched fuzzy hypotheses are the strongest of the four, and what makes a two-point argument weak is **support size**, not moment count. If (III) is recoverable, the grading is the thing to change. **Also settled:** `GradeSound` is automatic under nesting (0 violations in 198 cells; largest step up `1.7e-12`), so my recorded reason for refusing it as a field was **wrong** — and the eventual quantifier in `architecture_gradeSound_eventually` is weaker than needed. |
| 58 | **Long memory is free, and positivity buys an exponent.** The estimation floor is `3/(2n)` uniformly in the memory parameter; the moment body's entropy exponent is strictly below the hyperrectangle's at every admissible `α`. | standing (Parts XV–XVI) | `TransportedMinimax.lean` §LongMemoryGeometry: `transportedFloor_eq`, `transportedFloor_indep_of_memory`, `longMemoryVariance_strictMono`, `momentBody_entropy_exponent_lt` | Two panels with different memory lengths face the same estimation floor at the same sample size. And a sample-size calculation that treats a moment body as a hyperrectangle is conservative **by a power**, not by a constant. | Algebra **PROVED** over two named inputs (the conformal metric and the parameter variance), neither derived here. | **A discrepancy in the upstream gloss is recorded rather than resolved.** The upstream text calls the parameter variance one that "blows up" as memory lengthens while giving it as `δ³/(nε²)`; long memory is `δ → 0`, where that formula *shrinks*. `longMemoryVariance_strictMono` records which way the stated formula runs. The cancellation needs only the product and is untouched either way — which is why it is stated apart from the gloss — but the word and the formula disagree and someone holding the derivation should reconcile them. |
...
| 59 | **Hudson–Nei conversion `F_ST^H = 2·G_ST/(1+G_ST)`.** The two `F_ST` conventions differ by a non-constant factor: 2× as `G_ST → 0`, vanishing as `G_ST → 1`. | standing | `Conventions.trueHudsonFst_eq_of_neiGst`; `neiGst` (:155) and `hudsonFst` (:231) now carry the estimator each computes, and `hudsonFst_ne_trueHudsonFst` exhibits a separating point | No recalibration constant can absorb a convention mix-up, because the discrepancy is not a constant. A pipeline reporting `F_ST` without naming its convention is unreproducible up to twofold. | **PROVED, then SIMULATED — in that order.** Exact to `3.6e-16` over 16 cells, and inverting the identity predicts the Nei estimate from the Hudson estimate at 0.00% relative error, on data the identity was not fitted to. Upgraded from ASSERTED. | The corpus's arithmetic remains Nei's throughout; `hudsonFst` is used by the BBP spike calibration only. Nothing yet checks that a downstream consumer's convention matches its source's. |
| 60 | **Liability-threshold AUC.** `Φ((i − i_c)·√R² / √(v₁ + v₀))` for a dichotomised liability, against the equal-variance Gaussian form for a continuous outcome. | new this session | `PortabilityDrift.liabilityThresholdAUCFromExplainedR2` with `LiabilityThresholdRegime` carrying every degeneracy as an explicit field | Binary-trait AUC conversion. The equal-variance form is correct for a Gaussian *outcome* and wrong for a dichotomised liability; the regime is what selects between them, and neither supersedes the other. | **SIMULATED, both arms.** RMSE `0.0121`, bias `-0.0007` over 400 runs with one fitted parameter per trait, against an independently measured seed-to-seed noise floor of `0.0120` — it fits as well as anything can. The form it replaces is biased `-0.068` AUC with max error `0.120` on the same runs. | The wrong form was reached by a name-and-docstring that both said "liability-threshold" while the body delegated two levels down to the equal-variance formula. No instrument caught it: the output was a valid probability in range. See row 63. |
| 61 | **`founderFst` was heterozygosity loss, not `F_ST`.** `1 − (1−1/(2k))^t` is within-population heterozygosity decay; between-population `F_ST` after a split is `coalFst t Ne = t/(t + 2Nₑ)`. | **FALSIFIED as stated; correct for what it now says** | Renamed `DemographicHistory.founderHeterozygosityLoss`; sibling `heterozygosityLossVariableNe`; `PopulationGeneticsFoundations.heterozygosityLossDerived` and `heterozygosityLossFromDrift` are the same quantity, repaired earlier | Anything reading these as differentiation measures was on the wrong axis, not slightly off — different quantities with different limits. | **SIMULATED, FALSIFIED.** Ratio `2.001` against simulated between-population `F_ST` across 16 cells. At `Nₑ = 1000, t = 4000` the formula's cluster reports ≈`0` where the measurable `F_ST` is `0.50`. | **The check already encoded the right answer.** `validation/popgen_defs/battery2.py` compared the body against `truth = 1 − H/H₀`, simulated heterozygosity loss, and passed green; the `fstVariableNe` entry even carried `note="drift-only heterozygosity loss"`. Nothing in the pipeline compares a check's semantics to the name of what it checks. |
| 62 | **Off-zero invisibility.** | **FALSIFIED** | — | — | **SIMULATED, refuted at 18.2σ**, then independently by Hermite expansion. Two methods, same verdict. | Recorded here so the refutation is not rediscovered. |
| 63 | **Name-versus-body audit (population-genetics half).** A definition whose name asserts one quantity and whose body computes another. | new class, partially swept | Six renames landed: `founderFst`, `fstVariableNe`, `fstDerived`, `steppingStoneCoalescenceTime`, `ldCorrelationFromMigration`, and `effectiveSampleSize` repointed at `effectiveFisherInformation` | A reader is entitled to trust an identifier. `hudsonFst` computing Nei's `G_ST` was a twofold error available to anyone who trusted the name. | **BY READING ONLY.** 99 bodies read against their names across 8 files; 27 of 27 citation-bearing definitions match their sources term by term (Bhatia et al. 2013 eq. 10, Ohta–Kimura 1971, the `driftLDEquilibrium` fixed point solved by hand). Verdicts: 2 WRONG, 4 MISLEADING, 93 CORRECT. | **No instrument can see this class.** Range checks pass (the value is valid), mutation testing passes (a perturbed wrong body is wrong in the same coordinate), the build passes. The other half of the corpus — metrics, calibration, information-theoretic quantities — is unaudited. **And docstring accuracy is a separate, unrun audit:** 99 bodies were read but only ~35 docstrings, and row 60's defect was a case where name *and* docstring agreed and both were wrong about the arithmetic. |
| 64b | **Two theorems that are vacuous ON PURPOSE.** `DriftRegime.cluster_identities_hold_at_every_retention` and `ascertainment_artificial_loss` state that two quantities cannot be told apart. | standing | Marked in source as intentionally vacuous | The content IS the indistinguishability: a cross-check that holds at every value of the retention cannot detect a wrong retention, which is the blindness the section exists to exhibit. | **PROVED, and deliberately unfalsifiable.** | **An automated vacuity detector is WRONG on these two and only a reader can tell.** They are flagged by `equivalence.py` exactly as the vacuous ones are. Deleting them deletes the result. Any future sweep must read the marker before acting. |
| 64 | **43 rfl-candidate theorems.** Each states an equality between two names that evaluate identically on every sampled point. | open worklist | `validation/extract/equivalence.py` (`c0399f6a`); over 560 evaluable definitions it finds 65 classes agreeing everywhere, 7 agreeing only on the admissible box, and 43 theorems that may be `f x = f x` in two costumes | A headline claim that is an identity in disguise licenses nothing. `neutralAF_benchmark_discrimination_preserved_calibration_lost` was exactly this before it was fixed. | **GAP — the sweep is unfinished, not absent.** First pass classifies roughly 6–8 as genuine cross-domain identities worth keeping (`driftLDEquilibrium ≡ ibdRecurrenceFixedPoint`, log-loss regret `=` Bernoulli KL, `qst ≡ coalFst`), ~11 as pure aliases where one name is *defined as* the other, and the rest as correctly-named coincidences to record rather than change. | Agreement at finitely many points is a **hypothesis**, not a proof of equality; only the converse is solid, so definitions in *different* classes are definitely different. Nothing here proposes a deletion. |
| 65 | **The Arrow Theorem's reversal-odd tangent is empty for real scalar stationary data.** Stationarity alone gives `γ(−k) = γ(k)`, so the reversal-odd component vanishes identically and the ceiling characterization collapses to `r⊥ = 0 ⟺ η > 0`. | standing, **and narrower than stated** | `Calibrator/EnsembleChannel.lean`; measurement in `validation/differential/cluster/fam_arrow_reversal.py` | The arrow's extra content is not available from a real-valued stationary series. Anything hoping to detect time-reversal asymmetry in that setting is looking at a component that is zero for structural reasons, not small. | **SIMULATED, vacuity confirmed.** 48 layer/functional combinations at `φ = 0.6`, `n' = 400`, 60 000 reps: **zero above 4σ**, largest `\|t\| = 2.29`. The instrument is not blind — the control fires at `t = −632` from zero and matches its closed form to `t = −0.97`. | **The premise is carried as an assumption, not proved.** Real-valued stationarity is what makes the tangent empty; the ledger records the collapse, not a proof that the assumption always holds. Complex-valued or non-stationary settings are untouched. |
| 66 | **Cohort-partition law.** The risk-optimal partition sits at `n′* ≈ 3.29·τ` with `risk* ≈ 16.11/(N·τ)`. | new | `validation/differential/cluster/partition_curve.py` | How finely to partition a cohort is a computable optimum, not a convention. The optimum tracks the correlation length `τ` and nothing else. | **SIMULATED.** Closed form reproduces the published curve to worst relative error `1.7e-4`. `n′*/τ` stays in `3.27–3.50` and `N·τ·risk*` in `15.19–16.11` across `ρ = 0.5 … 0.9995`, a **1000× range in τ**. The optimum is flat to 6.1%, and moving the noise ratio 64× moves `n′*/τ` only `3.2 → 4.4`. | Not wired into any Lean definition; it lives in the validation tree only. The flatness is a feature for practice and means the constant is weakly identified — `3.29` should be quoted with its 6% plateau, not as a sharp value. |
| 67 | **Permeability family: finite-`m` law, sealing exponent, curvature, additivity.** | new | `Calibrator/Permeability.lean`; `validation/differential/cluster/fam_permeability.py` | Quantifies how much of an ensemble channel's signal crosses, and how that scales. | **SIMULATED throughout, with constants.** Finite-`m` permeability `6.031` against asymptotic `6.063` and conditional `6.009`, isotropic cell `4.911`; RMSE/predicted ratio falls `1.035 → 1.013 → 1.003` as `m` goes `50 → 200 → 800`, i.e. converging to the predicted law. Sealing exponent: local slope `2.0000` at the smallest `η`, secant `1.850` against closed form `1.785`. Additivity: fitted `a = 0.490 ± 0.006`, `b = 18.72 ± 0.05`, `c = 1.012 ± 0.009`, cross term `−12.0 ± 18.8` — consistent with zero, and the P4 control recovers a cross term of `175.9` when one is present. Curvature: `det J₃ = 0.4375`, `rank J₂ = 2`, rate slope `−0.575 ± 0.048`. | **RETRACTED ANOMALY, recorded rather than erased.** An earlier version of this row reported the `b` fit as `18.72 ± 0.05` against a claimed `19.39` — a `14σ` discrepancy "that nothing yet explains". Both the number and the attribution were an artifact, withdrawn by the agent that produced them. Cause: `b` is identified only by the 3 distinct `m` values, and a deduplication speed fix meant the deployment error was measured once per `m` and reused across 3 `n` and 3 `r⊥` values, so **9 of 27 rows shared one measurement while the least-squares fit treated them as independent**. Each measurement is a mean of 400 squared errors (7.1% relative SE), so the honest SE is `±0.79`, not `±0.048`, and the 0.667 shortfall is **0.84σ — consistent**. Confirmed by a repowered direct test (`m = 800`, 16 000 ensembles, resolving 3.4% at 3σ): square-root map `0.9970` of CRB, linearised `0.9954`, differing by `0.10σ` where a real Jensen effect would show `0.0340`. The closing law's middle coefficient is consistent with the stated `Σ1/(2pᵢ)`. Remaining: the κ-correction is `kappa_correction_tested: false`, and the deconvolution rate is NOT TESTED, numerically out of reach. |
