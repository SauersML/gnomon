...
/-- **The cross term is bounded by the geometric mean.**
    ...
    When effects are uncorrelated, R²_cross ≈ √(R²_A × R²_B). -/
theorem cross_term_bounded
    (r2_A r2_B r2_cross : ℝ)
    (h_A : 0 ≤ r2_A) (h_B : 0 ≤ r2_B)
    (h_bound : r2_cross ≤ Real.sqrt (r2_A * r2_B)) :
    r2_cross ≤ Real.sqrt (r2_A * r2_B) := h_bound
...
/-- **LAI-PGS is at least as good as single-ancestry PGS.**
    ...
    LAI-PGS reduces bias. -/
theorem lai_pgs_at_least_as_good
    (r2_standard r2_lai : ℝ)
    (h_better : r2_standard ≤ r2_lai)
    (h_nn : 0 ≤ r2_standard) :
    r2_standard ≤ r2_lai := h_better
...
theorem recent_admixture_benefits_more
    (gain_recent gain_ancient : ℝ)
    (h_recent_better : gain_ancient < gain_recent)
    (h_nn : 0 ≤ gain_ancient) :
    gain_ancient < gain_recent := h_recent_better
...
theorem equitable_pgs_overinvestment
    (n_needed n_proportional : ℝ)
    (h_overinvest : n_proportional < n_needed)
    (h_nn : 0 < n_proportional) :
    n_proportional < n_needed := h_overinvest
...
/-- **Universal portability is impossible.**
    ...
    The goal should be minimizing the maximum gap. -/
theorem universal_portability_impossible
    (n_populations : ℕ) (max_gap : ℝ)
    (h_many : 2 ≤ n_populations) (h_gap_pos : 0 < max_gap) :
    -- Some residual gap is inevitable
    0 < max_gap := h_gap_pos
