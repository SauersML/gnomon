...
theorem strong_selection_high_differentiation
    (s m k : ℝ) (h_s : 0 < s) (h_m : 0 < m) (h_k : 0 < k) (h_strong : k * m < s) :
    k / (k + 1) < selectionMigrationEquilibrium s m := by
  unfold selectionMigrationEquilibrium
  have h_denom : 0 < s + m := by linarith
  have h_k_plus_1 : 0 < k + 1 := by linarith
  rw [div_lt_div_iff₀ h_k_plus_1 h_denom]
  calc k * (s + m) = k * s + k * m := by ring
    _ < k * s + s := by linarith
    _ = s * (k + 1) := by ring
...
theorem weak_selection_low_differentiation
    (s m k : ℝ) (h_s : 0 < s) (h_m : 0 < m) (h_k : 0 < k) (h_weak : s < k * m) :
    selectionMigrationEquilibrium s m < k / (k + 1) := by
  unfold selectionMigrationEquilibrium
  have h_denom : 0 < s + m := by linarith
  rw [div_lt_div_iff₀ h_denom (by linarith)]
  calc s * (k + 1) = s * k + s := by ring
    _ < s * k + k * m := by linarith
    _ = k * (s + m) := by ring
...
theorem selected_loci_disproportionate_impact
    (fst_selected fst_neutral f_sel : ℝ)
    (h_higher : fst_neutral < fst_selected)
    (h_pos_f : 0 < f_sel) (h_lt_one : f_sel < 1)
    (h_nn : 0 ≤ fst_neutral) :
    -- The proportion of Fst from selected loci exceeds their fraction of the genome
    f_sel < (f_sel * fst_selected) / ((1 - f_sel) * fst_neutral + f_sel * fst_selected) := by
  have h_denom : 0 < (1 - f_sel) * fst_neutral + f_sel * fst_selected := by
    have h1 : 0 ≤ (1 - f_sel) * fst_neutral := mul_nonneg (by linarith) h_nn
    have h2 : 0 < f_sel * fst_selected := mul_pos h_pos_f (by linarith)
    linarith
  rw [lt_div_iff₀ h_denom]
  calc f_sel * ((1 - f_sel) * fst_neutral + f_sel * fst_selected)
    = f_sel * (1 - f_sel) * fst_neutral + f_sel * f_sel * fst_selected := by ring
    _ < f_sel * (1 - f_sel) * fst_selected + f_sel * f_sel * fst_selected := by
      have : f_sel * (1 - f_sel) > 0 := mul_pos h_pos_f (by linarith)
      have step : f_sel * (1 - f_sel) * fst_neutral < f_sel * (1 - f_sel) * fst_selected := mul_lt_mul_of_pos_left h_higher this
      linarith
    _ = f_sel * fst_selected := by ring
...
theorem genome_wide_fst_neutral_dominated
    (fst_gw fst_neutral fst_selected f_sel : ℝ)
    (h_gw : fst_gw = (1 - f_sel) * fst_neutral + f_sel * fst_selected)
    (h_pos : 0 ≤ f_sel) (_h_lt_one : f_sel ≤ 1)
    (h_neutral_nn : 0 ≤ fst_neutral)
    (_h_sel_higher : fst_neutral ≤ fst_selected) :
    fst_gw - fst_neutral ≤ f_sel * fst_selected := by
  rw [h_gw]
  have : (1 - f_sel) * fst_neutral + f_sel * fst_selected - fst_neutral =
      f_sel * (fst_selected - fst_neutral) := by ring
  rw [this]
  have : f_sel * (fst_selected - fst_neutral) = f_sel * fst_selected - f_sel * fst_neutral := by ring
  rw [this]
  have : 0 ≤ f_sel * fst_neutral := mul_nonneg h_pos h_neutral_nn
  linarith
