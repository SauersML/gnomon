...
structural phase-misspecification error when predictions perfectly match reality. -/
...
/-- Transport bias for a phase-aware haplotype model. -/
noncomputable def haplotypeTransportBias
    (freq_cis_target interaction_cis_target interaction_trans_target predicted_cis predicted_trans : ℝ) : ℝ :=
...
theorem haplotype_pgs_more_portable_for_cis
    ...
    (h_pred_cis : predicted_cis = interaction_cis)
    (h_pred_trans : predicted_trans = interaction_trans) :
    haplotypeTransportBias freq_cis_target interaction_cis interaction_trans predicted_cis predicted_trans <
      dosageTransportBias freq_cis_source freq_cis_target interaction_cis interaction_trans := by
  have h_zero : haplotypeTransportBias freq_cis_target interaction_cis interaction_trans predicted_cis predicted_trans = 0 := by
    rw [h_pred_cis, h_pred_trans]
    unfold haplotypeTransportBias
    have h_sub_self : averagePhaseInteraction freq_cis_target interaction_cis interaction_trans - averagePhaseInteraction freq_cis_target interaction_cis interaction_trans = 0 := by ring
    rw [h_sub_self]
    exact abs_zero
