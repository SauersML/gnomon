...
/-- A phase-aware haplotype predictor explicitly models the interaction within
cis and trans configurations, predicting the true specific interaction correctly. -/
...
noncomputable def haplotypePhasePredictionError (m : HaplotypePredictor) : ℝ :=
  m.freq_cis * (m.interaction_cis - m.pred_cis) ^ 2 +
    (1 - m.freq_cis) * (m.interaction_trans - m.pred_trans) ^ 2

theorem haplotypePhasePredictionError_eq_zero (m : HaplotypePredictor) :
    haplotypePhasePredictionError m = 0 := by
  unfold haplotypePhasePredictionError
  rw [m.h_pred_cis, m.h_pred_trans, sub_self, sub_self]
  ring
...
/-- A phase-aware haplotype transport predictor explicitly models predictions
when migrating from a source population to a target population where
configuration frequencies differ but effects remain consistent. -/
structure HaplotypeTransportPredictor where
  freq_cis_source : ℝ
  freq_cis_target : ℝ
  interaction_cis : ℝ
  interaction_trans : ℝ
...
frequencies differ. -/
noncomputable def haplotypeTransportBias (m : HaplotypeTransportPredictor) : ℝ :=
...
theorem haplotypeTransportBias_eq_zero (m : HaplotypeTransportPredictor) :
    haplotypeTransportBias m = 0 := by
  unfold haplotypeTransportBias
  rw [m.h_pred_cis, m.h_pred_trans, sub_self, abs_zero]
