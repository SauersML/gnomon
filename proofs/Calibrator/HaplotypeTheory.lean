...
noncomputable def haplotypeTransportBias
    (_freq_cis_target _interaction_cis _interaction_trans : ℝ) : ℝ :=
...
theorem haplotype_pgs_more_portable_for_cis
  ...
  have h_inner : haplotypeTransportBias freq_cis_target interaction_cis interaction_trans = 0 := by
    unfold haplotypeTransportBias; rw [sub_self, abs_zero]
  rw [h_inner]
  exact mul_pos
