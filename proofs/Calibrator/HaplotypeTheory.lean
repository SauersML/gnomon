...
theorem haplotype_pgs_more_portable_for_cis
  ...
  have h_hap_zero : haplotypeTransportBias freq_cis_target interaction_cis interaction_trans = 0 := by
    unfold haplotypeTransportBias; have h_inner : freq_cis_target * interaction_cis + (1 - freq_cis_target) * interaction_trans - (freq_cis_target * interaction_cis + (1 - freq_cis_target) * interaction_trans) = 0 := sub_self _; rw [h_inner, abs_zero]
  rw [h_hap_zero]
  exact mul_pos
