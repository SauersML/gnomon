...
frequencies differ. -/
noncomputable def haplotypeTransportBias (source_interaction target_interaction : ℝ) : ℝ :=
  |target_interaction - source_interaction|
...
theorem compound_het_not_captured_by_dosage
  ...
  have h_zero : haplotypePhasePredictionError interaction_cis interaction_cis = 0 := by unfold haplotypePhasePredictionError; ring
...
theorem haplotype_pgs_more_portable_for_cis
    ...
    (h_phase_gap : interaction_cis ≠ interaction_trans) :
    haplotypeTransportBias interaction_cis interaction_cis < dosageTransportBias
      freq_cis_source freq_cis_target interaction_cis interaction_trans := by
  have h_zero : haplotypeTransportBias interaction_cis interaction_cis = 0 := by unfold haplotypeTransportBias; rw [sub_self, abs_zero]
