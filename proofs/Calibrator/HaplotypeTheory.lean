...
structural phase-misspecification error. The model correctly uses `interaction_cis`
for cis configurations and `interaction_trans` for trans configurations. -/
...
frequencies differ. The target population is evaluated using its own
phase frequencies with the transported phase-specific effects. -/
