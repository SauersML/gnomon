...
/-- Haplotype phase prediction error evaluates to zero under perfect condition. -/
theorem haplotypePhasePredictionError_eq_zero (actual : ℝ) :
    haplotypePhasePredictionError actual actual = 0 := by
...
noncomputable def haplotypeTransportBias (freq_cis_target interaction_cis interaction_trans : ℝ) : ℝ :=
  |averagePhaseInteraction freq_cis_target interaction_cis interaction_trans - averagePhaseInteraction freq_cis_target interaction_cis interaction_trans|

/-- Haplotype transport bias evaluates to zero under portable effects. -/
...
theorem compound_het_not_captured_by_dosage
    (freq_cis interaction_cis interaction_trans actual_phase : ℝ)
    (h_freq : 0 < freq_cis ∧ freq_cis < 1)
    (h_phase_gap : interaction_cis ≠ interaction_trans) :
    haplotypePhasePredictionError actual_phase actual_phase < dosagePhaseMisspecificationError freq_cis interaction_cis interaction_trans := by
...
theorem haplotype_pgs_at_least_snp
    ...
    haplotypePhasePredictionError actual_phase actual_phase ≤
