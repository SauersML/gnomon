...
noncomputable def haplotypePhasePredictionError (freq_cis true_cis true_trans pred_cis pred_trans : ℝ) : ℝ :=
  freq_cis * (true_cis - pred_cis) ^ 2 + (1 - freq_cis) * (true_trans - pred_trans) ^ 2

theorem haplotype_perfect_prediction_zero (freq_cis interaction_cis interaction_trans : ℝ) :
...
theorem haplotype_perfect_transport_zero (freq_cis_target interaction_cis interaction_trans : ℝ) :
...
theorem compound_het_not_captured_by_dosage
  ...
  rw [dosagePhaseMisspecificationError_eq, haplotype_perfect_prediction_zero]
...
theorem haplotype_pgs_more_portable_for_cis
  ...
  rw [dosageTransportBias_eq, haplotype_perfect_transport_zero]
  exact mul_pos
