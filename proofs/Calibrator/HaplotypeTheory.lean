...
structural phase-misspecification error when predictions perfectly match interactions. -/
noncomputable def haplotypePhasePredictionError (freq_cis interaction_cis interaction_trans pred_cis pred_trans : ℝ) : ℝ :=
...
frequencies differ. -/
noncomputable def haplotypeTransportBias (interaction_cis interaction_trans pred_cis pred_trans : ℝ) : ℝ :=
  |interaction_cis - pred_cis| + |interaction_trans - pred_trans|

theorem haplotypeTransportBias_eq_zero (interaction_cis interaction_trans : ℝ) :
    haplotypeTransportBias interaction_cis interaction_trans interaction_cis interaction_trans = 0 := by
  unfold haplotypeTransportBias
  have h1 : interaction_cis - interaction_cis = 0 := sub_self _
  have h2 : interaction_trans - interaction_trans = 0 := sub_self _
  rw [h1, h2]
  have h_abs_zero : |(0 : ℝ)| = 0 := abs_zero
  rw [h_abs_zero, add_zero]
...
theorem haplotype_pgs_more_portable_for_cis
    ...
    (h_phase_gap : interaction_cis ≠ interaction_trans) :
    haplotypeTransportBias interaction_cis interaction_trans interaction_cis interaction_trans < dosageTransportBias
