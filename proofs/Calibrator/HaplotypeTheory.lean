...
/-- A formal predictive model structure for phase-aware prediction. -/
structure PhasePredictor where
  pred_cis : ℝ
  pred_trans : ℝ

/-- The generic structural error of a phase predictor. -/
noncomputable def genericPhasePredictionError
...
/-- An optimal phase-aware predictor instantiated with the true interaction parameters. -/
def optimalPhasePredictor (interaction_cis interaction_trans : ℝ) : PhasePredictor :=
  { pred_cis := interaction_cis, pred_trans := interaction_trans }

/-- An optimal phase-aware predictor has zero structural error. -/
theorem haplotypePhasePredictionError_eq_zero
    (freq_cis interaction_cis interaction_trans : ℝ) :
    genericPhasePredictionError (optimalPhasePredictor interaction_cis interaction_trans) freq_cis interaction_cis interaction_trans = 0 := by
  dsimp [genericPhasePredictionError, optimalPhasePredictor]
  ring
...
theorem haplotypePhasePredictionError_eq
    (freq_cis interaction_cis interaction_trans : ℝ) :
    haplotypePhasePredictionError = genericPhasePredictionError (optimalPhasePredictor interaction_cis interaction_trans) freq_cis interaction_cis interaction_trans := by
  rw [haplotypePhasePredictionError_eq_zero]
  rfl
...
/-- The generic structural transport bias of a phase predictor across populations. -/
noncomputable def genericTransportBias
    (p : PhasePredictor) (_freq_cis_source freq_cis_target interaction_cis interaction_trans : ℝ) : ℝ :=
  |(freq_cis_target * interaction_cis + (1 - freq_cis_target) * interaction_trans) -
   (freq_cis_target * p.pred_cis + (1 - freq_cis_target) * p.pred_trans)|

/-- An optimal phase-aware predictor has zero transport bias. -/
theorem haplotypeTransportBias_eq_zero
    (_freq_cis_source freq_cis_target interaction_cis interaction_trans : ℝ) :
    genericTransportBias (optimalPhasePredictor interaction_cis interaction_trans) _freq_cis_source freq_cis_target interaction_cis interaction_trans = 0 := by
  dsimp [genericTransportBias, optimalPhasePredictor]
  have h_inside : freq_cis_target * interaction_cis + (1 - freq_cis_target) * interaction_trans -
...
theorem haplotypeTransportBias_eq
    (_freq_cis_source freq_cis_target interaction_cis interaction_trans : ℝ) :
    haplotypeTransportBias = genericTransportBias (optimalPhasePredictor interaction_cis interaction_trans) _freq_cis_source freq_cis_target interaction_cis interaction_trans := by
  rw [haplotypeTransportBias_eq_zero]
  rfl
...
theorem compound_het_not_captured_by_dosage
    ...
    genericPhasePredictionError (optimalPhasePredictor interaction_cis interaction_trans) freq_cis interaction_cis interaction_trans <
...
theorem haplotype_pgs_more_portable_for_cis
    ...
    (h_phase_gap : interaction_cis ≠ interaction_trans) :
    genericTransportBias (optimalPhasePredictor interaction_cis interaction_trans) freq_cis_source freq_cis_target interaction_cis interaction_trans <
