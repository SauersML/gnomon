...
/-- Model representing parameters of a phase-aware haplotype predictor tracking cis/trans configuration. -/
...
structural phase-misspecification error. We compute this mathematically from generative
parameters rather than relying on a trivial 0 witness. -/
...
/-- Compatibility theorem to bridge the mathematical model to the rigorous API. -/
...
/-- Model for transporting phase-aware haplotype predictions across populations. -/
...
frequencies differ. Computed dynamically without a trivial witness. -/
...
/-- Compatibility theorem asserting that a structurally correct haplotype model achieves zero transport bias. -/
...
theorem compound_het_not_captured_by_dosage
  ...
  have h_rhs : 0 < m.freq_cis * (1 - m.freq_cis) * (m.interaction_cis - m.interaction_trans) ^ 2 := by
