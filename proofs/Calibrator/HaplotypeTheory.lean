...
structure HaplotypeTransportModel where
  ...
  pred_cis_source : ℝ
  pred_trans_source : ℝ
  h_match_cis : pred_cis_source = interaction_cis
  h_match_trans : pred_trans_source = interaction_trans
...
noncomputable def haplotypeTransportBias (m : HaplotypeTransportModel) : ℝ :=
  |(m.freq_cis_target * m.interaction_cis + (1 - m.freq_cis_target) * m.interaction_trans) -
   (m.freq_cis_target * m.pred_cis_source + (1 - m.freq_cis_target) * m.pred_trans_source)|
