...
noncomputable def haplotypePhasePredictionError
    (freq_cis pred_cis pred_trans interaction_cis interaction_trans : ℝ) : ℝ :=
...
theorem compound_het_not_captured_by_dosage
    (freq_cis pred_cis pred_trans interaction_cis interaction_trans : ℝ)
  ...
  have h_hap_zero : haplotypePhasePredictionError freq_cis pred_cis pred_trans interaction_cis interaction_trans = 0 := by
...
theorem haplotype_pgs_more_portable_for_cis
    (freq_cis_source freq_cis_target pred_cis pred_trans interaction_cis interaction_trans : ℝ)
    (h_pred_cis : pred_cis = interaction_cis)
    (h_pred_trans : pred_trans = interaction_trans)
    (h_freq_shift : freq_cis_source ≠ freq_cis_target)
    (h_phase_gap : interaction_cis ≠ interaction_trans) :
    haplotypeTransportBias freq_cis_target pred_cis pred_trans interaction_cis interaction_trans <
      dosageTransportBias freq_cis_source freq_cis_target interaction_cis interaction_trans := by
  rw [dosageTransportBias_eq]
  have h_hap_zero : haplotypeTransportBias freq_cis_target pred_cis pred_trans interaction_cis interaction_trans = 0 := by
    rw [h_pred_cis, h_pred_trans]
    unfold haplotypeTransportBias
    have h_sub : averagePhaseInteraction freq_cis_target interaction_cis interaction_trans - averagePhaseInteraction freq_cis_target interaction_cis interaction_trans = 0 := by ring
    rw [h_sub, abs_zero]
  rw [h_hap_zero]
  exact mul_pos
