...
structural phase-misspecification error when the cis and trans parameters
...
frequencies differ, assuming the parameters are estimated perfectly
in the source and applied to the target. -/
noncomputable def haplotypeTransportBias
    (freq_cis_target interaction_cis_target interaction_trans_target pred_cis_source pred_trans_source : ℝ) : ℝ :=
  |averagePhaseInteraction freq_cis_target interaction_cis_target interaction_trans_target -
    averagePhaseInteraction freq_cis_target pred_cis_source pred_trans_source|
