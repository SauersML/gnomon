...
structural phase-misspecification error when predictions perfectly match actuality. -/
...
theorem haplotypePhasePredictionError_zero
    (freq_cis true_interaction_cis true_interaction_trans pred_interaction_cis pred_interaction_trans : ℝ)
    (h_pred_cis : pred_interaction_cis = true_interaction_cis)
    (h_pred_trans : pred_interaction_trans = true_interaction_trans) :
    haplotypePhasePredictionError freq_cis true_interaction_cis true_interaction_trans pred_interaction_cis pred_interaction_trans = 0 := by
...
noncomputable def haplotypeTransportBias
  ...
  |averagePhaseInteraction freq_cis_target true_interaction_cis true_interaction_trans -
    averagePhaseInteraction freq_cis_target pred_interaction_cis pred_interaction_trans|

theorem haplotypeTransportBias_zero
    (freq_cis_target true_interaction_cis true_interaction_trans pred_interaction_cis pred_interaction_trans : ℝ)
    (h_pred_cis : pred_interaction_cis = true_interaction_cis)
    (h_pred_trans : pred_interaction_trans = true_interaction_trans) :
    haplotypeTransportBias freq_cis_target true_interaction_cis true_interaction_trans pred_interaction_cis pred_interaction_trans = 0 := by
  rw [h_pred_cis, h_pred_trans]
  unfold haplotypeTransportBias
  have h_sub : averagePhaseInteraction freq_cis_target true_interaction_cis true_interaction_trans - averagePhaseInteraction freq_cis_target true_interaction_cis true_interaction_trans = 0 := sub_self _
  rw [h_sub]
  exact abs_zero
...
theorem compound_het_not_captured_by_dosage
  ...
  rw [haplotypePhasePredictionError_zero _ _ _ _ _ h_pred_cis h_pred_trans]
...
theorem haplotype_pgs_more_portable_for_cis
    ...
    (h_pred_cis : pred_cis = interaction_cis)
    (h_pred_trans : pred_trans = interaction_trans) :
    haplotypeTransportBias freq_cis_target interaction_cis interaction_trans pred_cis pred_trans <
      dosageTransportBias freq_cis_source freq_cis_target interaction_cis interaction_trans := by
  rw [dosageTransportBias_eq]
  rw [haplotypeTransportBias_zero _ _ _ _ _ h_pred_cis h_pred_trans]
  exact mul_pos
