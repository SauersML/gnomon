...
structural phase-misspecification error when its predictions are perfect. -/
noncomputable def haplotypePhasePredictionError
    (freq_cis predicted_cis predicted_trans interaction_cis interaction_trans : ℝ) : ℝ :=
...
noncomputable def haplotypeTransportBias
    (freq_cis_target predicted_cis predicted_trans interaction_cis interaction_trans : ℝ) : ℝ :=
  |averagePhaseInteraction freq_cis_target interaction_cis interaction_trans -
