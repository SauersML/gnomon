...
/-- A phase-aware haplotype model that explicitly represents predicted
    and true phase-specific interaction effects. -/
structure HaplotypePhaseModel where
  ...
  h_match_cis : interaction_cis_pred = interaction_cis_true
  h_match_trans : interaction_trans_pred = interaction_trans_true

/-- A phase-aware haplotype predictor tracking cis/trans configuration computes
    its structural phase-misspecification error as the MSE between predicted
    and true configuration effects. -/
noncomputable def haplotypePhasePredictionError (m : HaplotypePhaseModel) : ℝ :=
  m.freq_cis * (m.interaction_cis_pred - m.interaction_cis_true) ^ 2 +
    (1 - m.freq_cis) * (m.interaction_trans_pred - m.interaction_trans_true) ^ 2

/-- When the predicted phase configurations exactly match the true effects,
    the structural prediction error vanishes. -/
...
/-- A phase-aware haplotype transport model tracking source-trained estimates
    and true target configuration effects. -/
structure HaplotypeTransportModel where
  freq_cis_target : ℝ
  interaction_cis_true_target : ℝ
  interaction_trans_true_target : ℝ
  interaction_cis_pred_source : ℝ
  interaction_trans_pred_source : ℝ
  h_portable_cis : interaction_cis_pred_source = interaction_cis_true_target
  h_portable_trans : interaction_trans_pred_source = interaction_trans_true_target

/-- A phase-aware haplotype model transports by capturing the exact configuration
    frequencies in the target, leaving bias only from misestimated portable effects. -/
noncomputable def haplotypeTransportBias (m : HaplotypeTransportModel) : ℝ :=
  |m.freq_cis_target * m.interaction_cis_pred_source + (1 - m.freq_cis_target) * m.interaction_trans_pred_source -
    (m.freq_cis_target * m.interaction_cis_true_target + (1 - m.freq_cis_target) * m.interaction_trans_true_target)|

/-- When the cis/trans effects themselves are portable, the structural
    transport bias vanishes. -/
theorem haplotypeTransportBias_eq_zero (m : HaplotypeTransportModel) :
  ...
  rw [m.h_portable_cis, m.h_portable_trans]
  ring_nf
  exact abs_zero
...
theorem compound_het_not_captured_by_dosage
    ...
    haplotypePhasePredictionError m < dosagePhaseMisspecificationError m.freq_cis interaction_cis interaction_trans := by
...
theorem haplotype_pgs_at_least_snp
    ...
    haplotypePhasePredictionError m ≤
      dosagePhaseMisspecificationError m.freq_cis interaction_cis interaction_trans := by
  rw [dosagePhaseMisspecificationError_eq, haplotypePhasePredictionError_eq_zero m]
...
theorem haplotype_pgs_more_portable_for_cis
    ...
    (freq_cis_source interaction_cis interaction_trans : ℝ)
    (h_freq_shift : freq_cis_source ≠ m.freq_cis_target)
    (h_phase_gap : interaction_cis ≠ interaction_trans) :
    haplotypeTransportBias m < dosageTransportBias
      freq_cis_source m.freq_cis_target interaction_cis interaction_trans := by
