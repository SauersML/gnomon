...
noncomputable def haplotypePhasePredictionError
    (freq_cis predicted_cis predicted_trans true_cis true_trans : ℝ) : ℝ :=
  freq_cis * (predicted_cis - true_cis) ^ 2 +
    (1 - freq_cis) * (predicted_trans - true_trans) ^ 2
...
noncomputable def haplotypeTransportBias
    (freq_cis_target predicted_cis predicted_trans true_cis true_trans : ℝ) : ℝ :=
