...
theorem haplotype_pgs_more_portable_for_cis
  ...
  have h_trans : haplotypeTransportBias freq_cis_source freq_cis_target interaction_cis interaction_trans = 0 := by
    unfold haplotypeTransportBias
    have h_zero : freq_cis_target * (interaction_cis - interaction_cis) + (1 - freq_cis_target) * (interaction_trans - interaction_trans) = 0 := by ring
    rw [h_zero, abs_zero]
  rw [dosageTransportBias_eq, h_trans]
  exact mul_pos
