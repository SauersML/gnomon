...
theorem haplotype_pgs_more_portable_for_cis
  ...
  have h_zero : haplotypeTransportBias freq_cis_source freq_cis_target interaction_cis interaction_trans = 0 := by
    ...
    have h1 : interaction_cis - interaction_cis = 0 := sub_self interaction_cis
    have h2 : interaction_trans - interaction_trans = 0 := sub_self interaction_trans
    rw [h1, h2]
    have h1_abs : |(0:ℝ)| = 0 := abs_zero
    rw [h1_abs]
    ring
  rw [h_zero]
  exact mul_pos
