...
noncomputable def haplotypePhasePredictionError
    (freq_cis true_cis true_trans model_cis model_trans : ℝ) : ℝ :=
  freq_cis * (true_cis - model_cis) ^ 2 + (1 - freq_cis) * (true_trans - model_trans) ^ 2

theorem haplotypePhasePredictionError_perfect_model
...
noncomputable def haplotypeTransportBias
    (freq_cis_target true_cis true_trans model_cis model_trans : ℝ) : ℝ :=
  |freq_cis_target * true_cis + (1 - freq_cis_target) * true_trans -
    (freq_cis_target * model_cis + (1 - freq_cis_target) * model_trans)|

theorem haplotypeTransportBias_perfect_model
...
theorem compound_het_not_captured_by_dosage
  ...
  rw [dosagePhaseMisspecificationError_eq, haplotypePhasePredictionError_perfect_model]
...
theorem haplotype_pgs_more_portable_for_cis
  ...
  rw [dosageTransportBias_eq, haplotypeTransportBias_perfect_model]
  exact mul_pos
