...
/-- Haplotype predictors model the distinct cis and trans effects explicitly. -/
structure HaplotypePredictor where
  cis_estimate : ℝ
  trans_estimate : ℝ

/-- A phase-aware haplotype predictor that tracks cis/trans configuration has
structural phase-misspecification error parameterized by its estimates. -/
noncomputable def haplotypePhasePredictionError
    (pred : HaplotypePredictor) (freq_cis interaction_cis interaction_trans : ℝ) : ℝ :=
  freq_cis * (interaction_cis - pred.cis_estimate) ^ 2 +
    (1 - freq_cis) * (interaction_trans - pred.trans_estimate) ^ 2

theorem haplotypePhasePredictionError_perfect
...
noncomputable def haplotypeTransportBias
    (pred : HaplotypePredictor) (freq_cis_target interaction_cis interaction_trans : ℝ) : ℝ :=
  |averagePhaseInteraction freq_cis_target interaction_cis interaction_trans -
    averagePhaseInteraction freq_cis_target pred.cis_estimate pred.trans_estimate|

theorem haplotypeTransportBias_perfect
...
theorem compound_het_not_captured_by_dosage
    ...
    haplotypePhasePredictionError ⟨interaction_cis, interaction_trans⟩ freq_cis interaction_cis interaction_trans < dosagePhaseMisspecificationError freq_cis interaction_cis interaction_trans := by
  rcases h_freq with ⟨h_freq_pos, h_freq_lt_one⟩
  rw [dosagePhaseMisspecificationError_eq, haplotypePhasePredictionError_perfect]
...
theorem haplotype_pgs_more_portable_for_cis
    ...
    (h_phase_gap : interaction_cis ≠ interaction_trans) :
    haplotypeTransportBias ⟨interaction_cis, interaction_trans⟩ freq_cis_target interaction_cis interaction_trans < dosageTransportBias
      freq_cis_source freq_cis_target interaction_cis interaction_trans := by
  rw [dosageTransportBias_eq, haplotypeTransportBias_perfect]
  exact mul_pos
