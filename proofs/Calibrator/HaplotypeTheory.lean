...
/-- A phase-aware haplotype predictor that tracks cis/trans configuration learns
specific estimates for the cis and trans configurations (pred_cis and pred_trans).
Its structural error when applied to a target population with true interaction
effects interaction_cis and interaction_trans is the variance of the residual error. -/
...
/-- A phase-aware haplotype model transports predicting pred_cis and pred_trans.
The structural bias on the average interaction prediction is the difference
between the true average interaction and predicted average interaction. -/
...
theorem compound_het_not_captured_by_dosage
  ...
  have h_hap_eq_zero : haplotypePhasePredictionError freq_cis interaction_cis interaction_trans interaction_cis interaction_trans = 0 := by
    unfold haplotypePhasePredictionError
    ring
  rw [dosagePhaseMisspecificationError_eq, h_hap_eq_zero]
...
theorem haplotype_pgs_more_portable_for_cis
  ...
  have h_hap_eq_zero : haplotypeTransportBias freq_cis_target interaction_cis interaction_trans interaction_cis interaction_trans = 0 := by
    unfold haplotypeTransportBias
    simp
  rw [dosageTransportBias_eq, h_hap_eq_zero]
  exact mul_pos
