...
noncomputable def haplotypeTransportBias
    (freq_cis_target true_cis true_trans predicted_cis predicted_trans : ℝ) : ℝ :=
...
theorem haplotypeTransportBias_eq_zero
    (freq_cis_target true_cis true_trans : ℝ) :
    haplotypeTransportBias freq_cis_target true_cis true_trans true_cis true_trans = 0 := by
  unfold haplotypeTransportBias
  have h_diff :
      (freq_cis_target * true_cis + (1 - freq_cis_target) * true_trans) -
      (freq_cis_target * true_cis + (1 - freq_cis_target) * true_trans) = 0 := by
    ring
  rw [h_diff, abs_zero]
