...
/-- A phase-aware haplotype predictor perfectly predicts the interaction effect
for each configuration, leaving zero structural error. -/
...
/-- A phase-aware haplotype model has zero structural bias because it applies
the configuration-specific effects properly in the target. -/
...
theorem compound_het_not_captured_by_dosage
  ...
  have h_hap_zero : haplotypePhasePredictionError freq_cis interaction_cis interaction_trans = 0 := by
...
theorem haplotype_pgs_more_portable_for_cis
  ...
  have h_hap_zero : haplotypeTransportBias freq_cis_target interaction_cis interaction_trans = 0 := by
    unfold haplotypeTransportBias averagePhaseInteraction; ring_nf; exact abs_zero
  rw [h_hap_zero]
  exact mul_pos
