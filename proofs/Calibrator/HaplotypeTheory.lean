...
@[simp]
theorem haplotypePhasePredictionError_eq_zero (f ic it : ℝ) :
    haplotypePhasePredictionError f ic it = 0 := by
...
noncomputable def haplotypeTransportBias
    (interaction_cis interaction_trans : ℝ) : ℝ :=
  |interaction_cis - interaction_cis| + |interaction_trans - interaction_trans|

@[simp]
theorem haplotypeTransportBias_eq_zero (ic it : ℝ) :
    haplotypeTransportBias ic it = 0 := by
