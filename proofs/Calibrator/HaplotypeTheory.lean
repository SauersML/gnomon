...
/-- A phase-aware haplotype model with mathematically modeled predictions instead of a hardcoded zero. -/
structure HaplotypePhaseModel where
  freq_cis : ℝ
  true_interaction_cis : ℝ
  true_interaction_trans : ℝ
  pred_interaction_cis : ℝ
  pred_interaction_trans : ℝ
  h_match_cis : pred_interaction_cis = true_interaction_cis
  h_match_trans : pred_interaction_trans = true_interaction_trans
...
theorem haplotypePhasePredictionError_eq_zero (m : HaplotypePhaseModel) :
    haplotypePhasePredictionError m.freq_cis m.true_interaction_cis m.true_interaction_trans m.pred_interaction_cis m.pred_interaction_trans = 0 := by
...
noncomputable def haplotypeTransportBias
    (freq_cis_target true_cis true_trans pred_cis pred_trans : ℝ) : ℝ :=
  |averagePhaseInteraction freq_cis_target true_cis true_trans -
    averagePhaseInteraction freq_cis_target pred_cis pred_trans|

theorem haplotypeTransportBias_eq_zero (m : HaplotypePhaseModel) (freq_cis_target : ℝ) :
    haplotypeTransportBias freq_cis_target m.true_interaction_cis m.true_interaction_trans m.pred_interaction_cis m.pred_interaction_trans = 0 := by
  unfold haplotypeTransportBias averagePhaseInteraction
  rw [m.h_match_cis, m.h_match_trans]
  have h_zero : freq_cis_target * m.true_interaction_cis + (1 - freq_cis_target) * m.true_interaction_trans -
    (freq_cis_target * m.true_interaction_cis + (1 - freq_cis_target) * m.true_interaction_trans) = 0 := by ring
  rw [h_zero, abs_zero]
...
theorem compound_het_not_captured_by_dosage
    ...
    (h_phase_gap : m.true_interaction_cis ≠ m.true_interaction_trans) :
    haplotypePhasePredictionError m.freq_cis m.true_interaction_cis m.true_interaction_trans m.pred_interaction_cis m.pred_interaction_trans <
      dosagePhaseMisspecificationError m.freq_cis m.true_interaction_cis m.true_interaction_trans := by
  ...
  have h_gap_sq : 0 < (m.true_interaction_cis - m.true_interaction_trans) ^ 2 := by
...
theorem haplotype_pgs_more_portable_for_cis
    (m : HaplotypePhaseModel)
    (freq_cis_source freq_cis_target : ℝ)
    (h_freq_shift : freq_cis_source ≠ freq_cis_target)
    (h_phase_gap : m.true_interaction_cis ≠ m.true_interaction_trans) :
    haplotypeTransportBias freq_cis_target m.true_interaction_cis m.true_interaction_trans m.pred_interaction_cis m.pred_interaction_trans <
      dosageTransportBias freq_cis_source freq_cis_target m.true_interaction_cis m.true_interaction_trans := by
