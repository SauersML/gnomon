...
structural phase-misspecification error when evaluated with exact effects. -/
...
theorem haplotypePhasePredictionError_exact
...
noncomputable def haplotypeTransportBias
    (_freq_cis_source freq_cis_target true_cis true_trans pred_cis pred_trans : ℝ) : ℝ :=
  |(freq_cis_target * true_cis + (1 - freq_cis_target) * true_trans) -
    (freq_cis_target * pred_cis + (1 - freq_cis_target) * pred_trans)|

theorem haplotypeTransportBias_exact
...
theorem compound_het_not_captured_by_dosage
  ...
  rw [dosagePhaseMisspecificationError_eq, haplotypePhasePredictionError_exact]
...
theorem haplotype_pgs_more_portable_for_cis
  ...
  rw [dosageTransportBias_eq, haplotypeTransportBias_exact]
  exact mul_pos
