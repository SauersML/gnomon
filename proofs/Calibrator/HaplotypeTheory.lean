...
noncomputable def haplotypeTransportBias
    (freq_cis_target interaction_cis_target interaction_trans_target interaction_cis_source interaction_trans_source : ℝ) : ℝ :=
...
theorem haplotypeTransportBias_perfect
  ...
  have h_cis : interaction_cis - interaction_cis = 0 := by ring
  have h_trans : interaction_trans - interaction_trans = 0 := by ring
  have h_abs_zero : |(0 : ℝ)| = 0 := abs_zero
  rw [h_cis, h_trans, h_abs_zero]
  ring
...
theorem compound_het_not_captured_by_dosage
  ...
  rw [haplotypePhasePredictionError_perfect]
...
theorem haplotype_pgs_more_portable_for_cis
  ...
  rw [haplotypeTransportBias_perfect]
  exact mul_pos
