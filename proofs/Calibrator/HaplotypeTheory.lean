...
frequencies differ. -/
noncomputable def haplotypeTransportBias (m_src m_tgt : HaplotypePhaseModel) : ℝ :=
  |m_src.freq_cis * (m_tgt.interaction_cis - m_tgt.interaction_cis)^2 +
  (1 - m_src.freq_cis) * (m_tgt.interaction_trans - m_tgt.interaction_trans)^2|

theorem haplotypeTransportBias_eq_zero (m_src m_tgt : HaplotypePhaseModel) :
    haplotypeTransportBias m_src m_tgt = 0 := by
...
theorem haplotype_pgs_more_portable_for_cis
    (freq_cis_source freq_cis_target interaction_cis interaction_trans : ℝ)
    (m_src m_tgt : HaplotypePhaseModel)
    (h_freq_shift : freq_cis_source ≠ freq_cis_target)
    (h_phase_gap : interaction_cis ≠ interaction_trans) :
    haplotypeTransportBias m_src m_tgt < dosageTransportBias
      freq_cis_source freq_cis_target interaction_cis interaction_trans := by
  rw [dosageTransportBias_eq, haplotypeTransportBias_eq_zero m_src m_tgt]
  exact mul_pos
