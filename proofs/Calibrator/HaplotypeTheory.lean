...
structure PhasePredictor where
  /-- Prediction for cis configuration -/
  pred_cis : ℝ
  /-- Prediction for trans configuration -/
  pred_trans : ℝ

/-- Mean squared error of a generic phase predictor. -/
noncomputable def PhasePredictor.misspecificationError (m : PhasePredictor)
    (freq_cis interaction_cis interaction_trans : ℝ) : ℝ :=
  freq_cis * (interaction_cis - m.pred_cis) ^ 2 +
    (1 - freq_cis) * (interaction_trans - m.pred_trans) ^ 2

/-- The optimal haplotype predictor perfectly learns the phase-specific interactions. -/
noncomputable def optimalHaplotypePredictor (interaction_cis interaction_trans : ℝ) : PhasePredictor :=
...
noncomputable def haplotypePhasePredictionError
    (freq_cis interaction_cis interaction_trans : ℝ) : ℝ :=
  (optimalHaplotypePredictor interaction_cis interaction_trans).misspecificationError
...
theorem haplotypePhasePredictionError_eq
  ...
  unfold haplotypePhasePredictionError PhasePredictor.misspecificationError optimalHaplotypePredictor
  ring
...
/-- Transport bias is the difference in average prediction when applying the
same predictor to a new population compared to the true average risk. -/
noncomputable def PhasePredictor.transportBias (m : PhasePredictor)
...
noncomputable def haplotypeTransportBias
    (freq_cis_target interaction_cis interaction_trans : ℝ) : ℝ :=
  (optimalHaplotypePredictor interaction_cis interaction_trans).transportBias
...
theorem haplotypeTransportBias_eq
  ...
  unfold haplotypeTransportBias PhasePredictor.transportBias optimalHaplotypePredictor
