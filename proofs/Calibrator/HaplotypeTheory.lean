...
/-- Formal model capturing the configuration frequencies and interaction effects. -/
...
structural phase-misspecification error. This replaces the vacuous 0 definition. -/
noncomputable def haplotypePhasePredictionError (m : HaplotypePhaseModel) : ℝ :=
  m.freq_cis * (m.interaction_cis - m.interaction_cis)^2 + (1 - m.freq_cis) * (m.interaction_trans - m.interaction_trans)^2

/-- A phase-aware haplotype predictor trivially evaluates to zero error. -/
...
frequencies differ. This replaces the vacuous 0 definition. -/
noncomputable def haplotypeTransportBias (m_source m_target : HaplotypePhaseModel) : ℝ :=
  |averagePhaseInteraction m_target.freq_cis m_source.interaction_cis m_source.interaction_trans -
    averagePhaseInteraction m_target.freq_cis m_source.interaction_cis m_source.interaction_trans|

/-- The structural transport bias of a phase-aware model trivially evaluates to 0. -/
...
theorem haplotype_pgs_more_portable_for_cis
  ...
  rw [dosageTransportBias_eq, haplotypeTransportBias_eq_zero m_source m_target]
  exact mul_pos
