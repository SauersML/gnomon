...
/-- Structural constraints for a phase-aware haplotype model. -/
...
/-- Structural constraints for haplotype transport without bias. -/
structure HaplotypeTransportModel where
  interaction_source : ℝ
  interaction_target : ℝ
  h_match : interaction_source = interaction_target
...
noncomputable def haplotypeTransportBias (m : HaplotypeTransportModel) : ℝ :=
  |m.interaction_target - m.interaction_source|
...
theorem haplotypeTransportBias_eq_zero (m : HaplotypeTransportModel) :
  ...
  rw [m.h_match, sub_self, abs_zero]
