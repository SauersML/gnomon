...
structural phase-misspecification error when it perfectly captures the interaction. -/
...
frequencies differ, and the model correctly captures the source interactions. -/
...
theorem haplotype_pgs_more_portable_for_cis
  ...
  have h_haplotype_zero : haplotypeTransportBias freq_cis_source freq_cis_target interaction_cis interaction_trans interaction_cis interaction_trans = 0 := by
    unfold haplotypeTransportBias; simp
  rw [h_haplotype_zero]
  exact mul_pos
