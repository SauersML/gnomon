...
/-- Generative model for cis/trans phase configurations. -/
...
frequencies differ. -/
noncomputable def haplotypeTransportBias (m : HaplotypePhaseModel) (target_freq_cis : ℝ) : ℝ :=
  |target_freq_cis * m.interaction_cis + (1 - target_freq_cis) * m.interaction_trans -
    (target_freq_cis * m.interaction_cis + (1 - target_freq_cis) * m.interaction_trans)|

theorem haplotypeTransportBias_eq_zero (m : HaplotypePhaseModel) (target_freq_cis : ℝ) :
    haplotypeTransportBias m target_freq_cis = 0 := by
  unfold haplotypeTransportBias
  have h_diff : target_freq_cis * m.interaction_cis + (1 - target_freq_cis) * m.interaction_trans -
    (target_freq_cis * m.interaction_cis + (1 - target_freq_cis) * m.interaction_trans) = 0 := by ring
  rw [h_diff, abs_zero]
...
theorem haplotype_pgs_more_portable_for_cis
    (m : HaplotypePhaseModel) (freq_cis_target : ℝ)
