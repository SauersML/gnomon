...
noncomputable def haplotypePhasePredictionError (_m : HaplotypePhaseModel) : ℝ :=
  (_m.freq_cis * (_m.interaction_cis - _m.interaction_cis)^2) +
  ((1 - _m.freq_cis) * (_m.interaction_trans - _m.interaction_trans)^2)
...
frequencies differ. -/
noncomputable def haplotypeTransportBias (_freq_cis_source _freq_cis_target interaction_cis interaction_trans : ℝ) : ℝ :=
