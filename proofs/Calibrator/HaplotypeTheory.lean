...
theorem compound_het_not_captured_by_dosage
  ...
  have h_zero : haplotypePhasePredictionError freq_cis interaction_cis interaction_trans = 0 := by
...
theorem haplotype_pgs_more_portable_for_cis
  ...
  have h_zero : haplotypeTransportBias freq_cis_source freq_cis_target interaction_cis interaction_trans = 0 := by
    ...
    simp
  rw [h_zero]
  have h_eq : |(freq_cis_target - freq_cis_source) * (interaction_cis - interaction_trans)| = |freq_cis_target - freq_cis_source| * |interaction_cis - interaction_trans| := abs_mul _ _
  rw [← h_eq]
  exact abs_pos.mpr (mul_ne_zero (sub_ne_zero.mpr h_freq_shift.symm) (sub_ne_zero.mpr h_phase_gap))
