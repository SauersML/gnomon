...
has structural phase-misspecification error dependent on how well it estimates
the true cis and trans interactions. -/
...
/-- A perfectly calibrated phase-aware predictor has zero structural error. -/
...
/-- A phase-aware haplotype model transports with a bias that depends on
how well it predicts the true cis and trans interactions, evaluated over the
target frequency distribution versus the source frequency distribution. -/
noncomputable def haplotypeTransportBias
    (freq_cis_source freq_cis_target interaction_cis interaction_trans pred_cis pred_trans : ℝ) : ℝ :=
  |(freq_cis_target * (interaction_cis - pred_cis) + (1 - freq_cis_target) * (interaction_trans - pred_trans)) -
   (freq_cis_source * (interaction_cis - pred_cis) + (1 - freq_cis_source) * (interaction_trans - pred_trans))|

/-- A phase-aware haplotype model transports without this structural bias when
it perfectly captures the true cis and trans effects. -/
