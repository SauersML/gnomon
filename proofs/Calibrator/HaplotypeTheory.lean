...
structural phase-misspecification error when it perfectly predicts the interaction effects. -/
...
frequencies differ. This measures the difference between the true expectation in the target population and the predicted expectation evaluated in the target population. -/
noncomputable def haplotypeTransportBias
    (freq_cis_target interaction_cis interaction_trans est_cis est_trans : ℝ) : ℝ :=
  |averagePhaseInteraction freq_cis_target interaction_cis interaction_trans -
    averagePhaseInteraction freq_cis_target est_cis est_trans|
