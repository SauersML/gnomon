...
/-- A generalized phase predictor that predicts effects for cis and trans
configurations. -/
structure PhasePredictor where
  /-- Predicted interaction effect for cis configuration -/
  pred_cis : ℝ
  /-- Predicted interaction effect for trans configuration -/
  pred_trans : ℝ

/-- The dosage-only predictor uses the population-average interaction for
both configurations. -/
noncomputable def dosagePredictor
    (freq_cis interaction_cis interaction_trans : ℝ) : PhasePredictor :=
  { pred_cis := averagePhaseInteraction freq_cis interaction_cis interaction_trans,
    pred_trans := averagePhaseInteraction freq_cis interaction_cis interaction_trans }

/-- The ideal haplotype predictor accurately captures both cis and trans effects. -/
noncomputable def idealHaplotypePredictor
    (interaction_cis interaction_trans : ℝ) : PhasePredictor :=
  { pred_cis := interaction_cis,
    pred_trans := interaction_trans }

/-- Expected squared prediction error for a given phase predictor. -/
noncomputable def PhasePredictor.misspecificationError
    (p : PhasePredictor) (freq_cis interaction_cis interaction_trans : ℝ) : ℝ :=
  freq_cis * (interaction_cis - p.pred_cis) ^ 2 +
    (1 - freq_cis) * (interaction_trans - p.pred_trans) ^ 2

/-- Transport bias from carrying a source-trained predictor into a
target population, defined as the difference in average PREDICTED effects
across populations due to configuration frequency shifts, compared to the
difference in average TRUE effects. -/
noncomputable def PhasePredictor.transportBias
    (p : PhasePredictor) (freq_cis_source freq_cis_target interaction_cis interaction_trans : ℝ) : ℝ :=
  |(averagePhaseInteraction freq_cis_target interaction_cis interaction_trans -
   ...
   (averagePhaseInteraction freq_cis_target p.pred_cis p.pred_trans -
    averagePhaseInteraction freq_cis_source p.pred_cis p.pred_trans)|
...
theorem dosagePhaseMisspecificationError_eq
    ...
    (dosagePredictor freq_cis interaction_cis interaction_trans).misspecificationError
      freq_cis interaction_cis interaction_trans =
      freq_cis * (1 - freq_cis) * (interaction_cis - interaction_trans) ^ 2 := by
  unfold PhasePredictor.misspecificationError dosagePredictor averagePhaseInteraction
  ring
...
theorem haplotypePhasePredictionError_eq
    (freq_cis interaction_cis interaction_trans : ℝ) :
    (idealHaplotypePredictor interaction_cis interaction_trans).misspecificationError
      freq_cis interaction_cis interaction_trans = 0 := by
  unfold PhasePredictor.misspecificationError idealHaplotypePredictor
  ring
...
theorem dosageTransportBias_eq
    ...
    (dosagePredictor freq_cis_source interaction_cis interaction_trans).transportBias
  ...
  unfold PhasePredictor.transportBias dosagePredictor averagePhaseInteraction
  have h_factor :
      freq_cis_target * interaction_cis + (1 - freq_cis_target) * interaction_trans -
        ...
        (freq_cis_target * (freq_cis_source * interaction_cis + (1 - freq_cis_source) * interaction_trans) +
          ...
          (freq_cis_source * (freq_cis_source * interaction_cis + (1 - freq_cis_source) * interaction_trans) +
            (1 - freq_cis_source) * (freq_cis_source * interaction_cis + (1 - freq_cis_source) * interaction_trans))) =
        (freq_cis_target - freq_cis_source) * (interaction_cis - interaction_trans) := by
    ring
  rw [h_factor, abs_mul]
...
theorem haplotypeTransportBias_eq
    (freq_cis_source freq_cis_target interaction_cis interaction_trans : ℝ) :
    (idealHaplotypePredictor interaction_cis interaction_trans).transportBias
      freq_cis_source freq_cis_target interaction_cis interaction_trans = 0 := by
  unfold PhasePredictor.transportBias idealHaplotypePredictor averagePhaseInteraction
  have h_inner :
    freq_cis_target * interaction_cis + (1 - freq_cis_target) * interaction_trans -
      (freq_cis_source * interaction_cis + (1 - freq_cis_source) * interaction_trans) -
      (freq_cis_target * interaction_cis + (1 - freq_cis_target) * interaction_trans -
        (freq_cis_source * interaction_cis + (1 - freq_cis_source) * interaction_trans)) = 0 := by ring
  rw [h_inner]
  exact abs_zero
...
theorem compound_het_not_captured_by_dosage
    ...
    (idealHaplotypePredictor interaction_cis interaction_trans).misspecificationError
      freq_cis interaction_cis interaction_trans <
    (dosagePredictor freq_cis interaction_cis interaction_trans).misspecificationError
      freq_cis interaction_cis interaction_trans := by
