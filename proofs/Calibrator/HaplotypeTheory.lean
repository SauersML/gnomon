...
/-- A phase-aware haplotype predictor that tracks cis/trans configuration predicts
the exact configuration effect. The structural phase-misspecification error
is the expected squared difference between the true effect and the predicted effect. -/
...
frequencies differ. The bias is the difference between the true average target effect
and the average predicted effect in the target. -/
