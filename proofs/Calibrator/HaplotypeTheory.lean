...
/-- Residual variance from a phase-aware predictor relative to the true
    interaction effects. An optimal predictor will have zero error. -/
noncomputable def haplotypePhasePredictionError
    (freq_cis interaction_cis interaction_trans : ℝ) (pred : PhasePredictor) : ℝ :=
...
frequencies differ. We define the transport bias of any phase predictor as
the absolute difference between the true average phase interaction in the target
population and the average predicted interaction. -/
noncomputable def haplotypeTransportBias
    (freq_cis_target interaction_cis interaction_trans : ℝ) (pred : PhasePredictor) : ℝ :=
  |averagePhaseInteraction freq_cis_target interaction_cis interaction_trans -
    averagePhaseInteraction freq_cis_target pred.pred_cis pred.pred_trans|

/-- An optimally-parameterized phase predictor has zero phase-misspecification error. -/
theorem haplotypePhasePredictionError_eq_zero
    (freq_cis interaction_cis interaction_trans : ℝ) :
    haplotypePhasePredictionError freq_cis interaction_cis interaction_trans (optimalPhasePredictor interaction_cis interaction_trans) = 0 := by
  unfold haplotypePhasePredictionError optimalPhasePredictor
  ring

/-- An optimally-parameterized phase predictor transports without structural bias. -/
theorem haplotypeTransportBias_eq_zero
    (freq_cis_target interaction_cis interaction_trans : ℝ) :
    haplotypeTransportBias freq_cis_target interaction_cis interaction_trans (optimalPhasePredictor interaction_cis interaction_trans) = 0 := by
  unfold haplotypeTransportBias optimalPhasePredictor averagePhaseInteraction
  ring_nf
  exact abs_zero
...
theorem compound_het_not_captured_by_dosage
    ...
    haplotypePhasePredictionError freq_cis interaction_cis interaction_trans (optimalPhasePredictor interaction_cis interaction_trans) < dosagePhaseMisspecificationError freq_cis interaction_cis interaction_trans := by
...
theorem haplotype_pgs_at_least_snp
    ...
    haplotypePhasePredictionError freq_cis interaction_cis interaction_trans (optimalPhasePredictor interaction_cis interaction_trans) ≤
...
theorem haplotype_pgs_more_portable_for_cis
    ...
    (h_phase_gap : interaction_cis ≠ interaction_trans) :
    haplotypeTransportBias freq_cis_target interaction_cis interaction_trans (optimalPhasePredictor interaction_cis interaction_trans) < dosageTransportBias
