...
/-- A phase-aware haplotype predictor that explicitly models the interaction
effect for cis and trans configurations. -/
structure PhaseAwarePredictor where
  pred_cis : ℝ
  pred_trans : ℝ

/-- A phase-aware haplotype predictor that tracks cis/trans configuration has no
structural phase-misspecification error when predictions are perfect. -/
...
/-- A perfect phase-aware haplotype predictor has zero prediction error. -/
theorem haplotypePhasePredictionError_perfect_eq_zero
    (interaction_cis interaction_trans freq_cis : ℝ) :
    haplotypePhasePredictionError ⟨interaction_cis, interaction_trans⟩ freq_cis interaction_cis interaction_trans = 0 := by
  unfold haplotypePhasePredictionError
  ring

/-- A perfect phase-aware haplotype predictor has zero transport bias. -/
theorem haplotypeTransportBias_perfect_eq_zero
    (interaction_cis interaction_trans freq_cis_target : ℝ) :
    haplotypeTransportBias ⟨interaction_cis, interaction_trans⟩ freq_cis_target interaction_cis interaction_trans = 0 := by
...
theorem compound_het_not_captured_by_dosage
    ...
    haplotypePhasePredictionError ⟨interaction_cis, interaction_trans⟩ freq_cis interaction_cis interaction_trans <
  ...
  rw [dosagePhaseMisspecificationError_eq, haplotypePhasePredictionError_perfect_eq_zero]
...
theorem haplotype_pgs_more_portable_for_cis
    ...
    (h_phase_gap : interaction_cis ≠ interaction_trans) :
    haplotypeTransportBias ⟨interaction_cis, interaction_trans⟩ freq_cis_target interaction_cis interaction_trans <
      dosageTransportBias freq_cis_source freq_cis_target interaction_cis interaction_trans := by
  rw [dosageTransportBias_eq, haplotypeTransportBias_perfect_eq_zero]
  exact mul_pos
