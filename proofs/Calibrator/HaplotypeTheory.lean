...
frequencies differ. -/
noncomputable def haplotypeTransportBias (target_effect source_effect : ℝ) : ℝ :=
...
theorem haplotype_pgs_more_portable_for_cis
    (freq_cis_source freq_cis_target interaction_cis interaction_trans target_effect source_effect : ℝ)
    (h_freq_shift : freq_cis_source ≠ freq_cis_target)
    (h_phase_gap : interaction_cis ≠ interaction_trans)
    (h_effect_eq : target_effect = source_effect) :
    haplotypeTransportBias target_effect source_effect < dosageTransportBias
      freq_cis_source freq_cis_target interaction_cis interaction_trans := by
  have h_zero : haplotypeTransportBias target_effect source_effect = 0 := by
    rw [h_effect_eq]
    unfold haplotypeTransportBias
    ring_nf
    exact abs_zero
  rw [h_zero, dosageTransportBias_eq]
  exact mul_pos
