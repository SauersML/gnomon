...
noncomputable def haplotypeTransportBias
  ...
  freq_cis_target * |interaction_cis_target - predicted_cis| +
    (1 - freq_cis_target) * |interaction_trans_target - predicted_trans|
...
theorem haplotype_pgs_more_portable_for_cis
  ...
  have h_zero : haplotypeTransportBias freq_cis_target interaction_cis interaction_trans predicted_cis predicted_trans = 0 := by
    ...
    have h_abs1 : |interaction_cis - interaction_cis| = 0 := by
      have h_sub : interaction_cis - interaction_cis = 0 := sub_self _
      rw [h_sub, abs_zero]
    have h_abs2 : |interaction_trans - interaction_trans| = 0 := by
      have h_sub : interaction_trans - interaction_trans = 0 := sub_self _
      rw [h_sub, abs_zero]
    rw [h_abs1, h_abs2]
    ring
  rw [h_zero]
