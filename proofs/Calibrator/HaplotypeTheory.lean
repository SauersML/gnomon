...
/-- A phase-aware haplotype model tracks cis and trans states and can model
...
structural phase-misspecification error because it accurately matches both states. -/
...
frequencies differ, tracking target configuration perfectly. -/
...
theorem compound_het_not_captured_by_dosage
  ...
  rw [haplotypePhasePredictionError_eq_zero ⟨freq_cis, interaction_cis, interaction_trans⟩]
...
theorem haplotype_pgs_more_portable_for_cis
  ...
  rw [haplotypeTransportBias_eq_zero freq_cis_source freq_cis_target interaction_cis interaction_trans]
  exact mul_pos
