...
noncomputable def haplotypePhasePredictionError
  ...
  freq_cis * (actual_cis - pred_cis) ^ 2 +
    (1 - freq_cis) * (actual_trans - pred_trans) ^ 2
...
theorem haplotype_pgs_more_portable_for_cis
  ...
  have h_zero : haplotypeTransportBias freq_cis_target interaction_cis interaction_trans pred_cis pred_trans = 0 := by
    ...
    have h_sub_zero : averagePhaseInteraction freq_cis_target interaction_cis interaction_trans - averagePhaseInteraction freq_cis_target interaction_cis interaction_trans = 0 := by
      exact sub_self _
    rw [h_sub_zero]
    exact abs_zero
  rw [h_zero]
