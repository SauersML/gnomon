...
noncomputable def haplotypePhasePredictionError
  ...
  freq_cis * (pred_cis - interaction_cis) ^ 2 +
    (1 - freq_cis) * (pred_trans - interaction_trans) ^ 2
...
noncomputable def haplotypeTransportBias
  ...
  |freq_cis_target * (pred_cis - interaction_cis) + (1 - freq_cis_target) * (pred_trans - interaction_trans)|
...
theorem haplotype_pgs_more_portable_for_cis
  ...
  have h_inner : haplotypeTransportBias freq_cis_target interaction_cis interaction_trans interaction_cis interaction_trans = 0 := by
    unfold haplotypeTransportBias
    have h_sub : interaction_cis - interaction_cis = 0 := sub_self _
    have h_sub2 : interaction_trans - interaction_trans = 0 := sub_self _
    rw [h_sub, h_sub2]
    ring_nf
    exact abs_zero
  rw [h_inner]
