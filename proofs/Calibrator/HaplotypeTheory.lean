...
noncomputable def haplotypePhasePredictionError
    (actual_cis pred_cis actual_trans pred_trans freq_cis : ℝ) : ℝ :=
  freq_cis * (actual_cis - pred_cis) ^ 2 + (1 - freq_cis) * (actual_trans - pred_trans) ^ 2
...
noncomputable def haplotypeTransportBias
    (freq_cis_target : ℝ)
    (actual_cis actual_trans : ℝ)
    (pred_cis pred_trans : ℝ) : ℝ :=
...
theorem compound_het_not_captured_by_dosage
    ...
    haplotypePhasePredictionError interaction_cis interaction_cis interaction_trans interaction_trans freq_cis < dosagePhaseMisspecificationError freq_cis interaction_cis interaction_trans := by
  rcases h_freq with ⟨h_freq_pos, h_freq_lt_one⟩
  rw [dosagePhaseMisspecificationError_eq]
  have h_zero : haplotypePhasePredictionError interaction_cis interaction_cis interaction_trans interaction_trans freq_cis = 0 := by
...
theorem haplotype_pgs_at_least_snp
    ...
    haplotypePhasePredictionError interaction_cis interaction_cis interaction_trans interaction_trans freq_cis ≤
