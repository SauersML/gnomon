...
predicts exactly `interaction_cis` for cis and `interaction_trans` for trans.
Its structural phase-misspecification error is thus based on exact matching. -/
...
/-- A phase-aware haplotype model transports without this structural bias because it applies
the correct effect (`interaction_cis` or `interaction_trans`) to each configuration
encountered in the target population. Thus, its target-population expectation
perfectly matches the target-population true expectation. -/
