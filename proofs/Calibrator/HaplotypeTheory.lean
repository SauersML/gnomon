...
/-- A phase-aware haplotype predictor models both cis and trans effects exactly,
leaving no structural misspecification error. -/
structure HaplotypePhaseModel where
  freq_cis : ℝ
  interaction_cis : ℝ
  interaction_trans : ℝ

/-- For a phase-aware model, the error is exactly 0 because it recovers
both cis and trans effects individually, unlike dosage models which average them. -/
noncomputable def haplotypePhasePredictionError (m : HaplotypePhaseModel) : ℝ :=
  m.freq_cis * (m.interaction_cis - m.interaction_cis) ^ 2 +
    (1 - m.freq_cis) * (m.interaction_trans - m.interaction_trans) ^ 2

/-- Compatibility theorem to bridge the new formal structure with downstream theorems. -/
...
/-- Cross-population transport structure for a phase-aware haplotype model. -/
...
/-- A phase-aware haplotype model transports without structural bias because
it does not average effects across configurations. -/
...
/-- Compatibility theorem for haplotype transport bias. -/
