...
theorem compound_het_not_captured_by_dosage
  ...
  have h_zero : freq_cis * 0 ^ 2 + (1 - freq_cis) * 0 ^ 2 = 0 := by ring
  rw [h_zero]
...
theorem haplotype_pgs_more_portable_for_cis
  ...
  have h_inner_cis : interaction_cis - interaction_cis = 0 := sub_self _
  have h_inner_trans : interaction_trans - interaction_trans = 0 := sub_self _
  have h_zero : freq_cis_target * interaction_cis + (1 - freq_cis_target) * interaction_trans - (freq_cis_target * interaction_cis + (1 - freq_cis_target) * interaction_trans) = 0 := sub_self _
  rw [h_zero, abs_zero]
  exact mul_pos
