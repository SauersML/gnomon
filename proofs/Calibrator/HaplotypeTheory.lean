...
theorem haplotypeTransportBias_eq_zero (m_source m_target : HaplotypePhaseModel)
    (h_same_cis : m_source.interaction_cis = m_target.interaction_cis)
    (h_same_trans : m_source.interaction_trans = m_target.interaction_trans) :
    haplotypeTransportBias m_source m_target = 0 := by
  unfold haplotypeTransportBias
  rw [h_same_cis, h_same_trans, sub_self, abs_zero]
...
theorem haplotype_pgs_more_portable_for_cis
  ...
  rw [dosageTransportBias_eq, haplotypeTransportBias_eq_zero m_source m_target h_same_cis h_same_trans]
  exact mul_pos
