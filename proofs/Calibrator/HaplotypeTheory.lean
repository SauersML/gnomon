...
theorem haplotype_pgs_more_portable_for_cis
  ...
  have h_inner : haplotypeTransportBias freq_cis_target interaction_cis interaction_trans = 0 := by
    ...
    have h_val : freq_cis_target * interaction_cis + (1 - freq_cis_target) * interaction_trans - (freq_cis_target * interaction_cis + (1 - freq_cis_target) * interaction_trans) = 0 := by ring
    rw [h_val, abs_zero]
  rw [h_inner]
  exact mul_pos
