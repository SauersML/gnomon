...
noncomputable def haplotypeTransportBias
    (freq_cis_target : ℝ)
    (pred_cis_source pred_cis_target pred_trans_source pred_trans_target : ℝ) : ℝ :=
  freq_cis_target * |pred_cis_target - pred_cis_source| +
    (1 - freq_cis_target) * |pred_trans_target - pred_trans_source|
...
theorem compound_het_not_captured_by_dosage
  ...
  have h_haplotype_zero : haplotypePhasePredictionError freq_cis interaction_cis interaction_trans interaction_cis interaction_trans = 0 := by
    unfold haplotypePhasePredictionError
    ring
  rw [dosagePhaseMisspecificationError_eq, h_haplotype_zero]
...
theorem haplotype_pgs_more_portable_for_cis
  ...
  have h_haplotype_zero : haplotypeTransportBias freq_cis_target interaction_cis interaction_cis interaction_trans interaction_trans = 0 := by
    unfold haplotypeTransportBias
    have h_abs_zero : |interaction_cis - interaction_cis| = 0 := by rw [sub_self, abs_zero]
    have h_abs_zero_trans : |interaction_trans - interaction_trans| = 0 := by rw [sub_self, abs_zero]
    rw [h_abs_zero, h_abs_zero_trans]
    ring
  rw [dosageTransportBias_eq, h_haplotype_zero]
  exact mul_pos
