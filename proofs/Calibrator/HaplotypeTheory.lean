...
theorem compound_het_not_captured_by_dosage
  ...
  have h_inner : freq_cis * (interaction_cis - interaction_cis) ^ 2 + (1 - freq_cis) * (interaction_trans - interaction_trans) ^ 2 = 0 := by simp
  rw [h_inner]
...
theorem haplotype_pgs_more_portable_for_cis
  ...
  have h_inner : |averagePhaseInteraction freq_cis_target interaction_cis interaction_trans - averagePhaseInteraction freq_cis_target interaction_cis interaction_trans| = 0 := by simp
  rw [h_inner]
  exact mul_pos
