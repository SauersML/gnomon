...
noncomputable def haplotypePhasePredictionError
    (freq_cis interaction_cis interaction_trans prediction_cis prediction_trans : ℝ) : ℝ :=
  freq_cis * (interaction_cis - prediction_cis) ^ 2 +
    (1 - freq_cis) * (interaction_trans - prediction_trans) ^ 2
...
noncomputable def haplotypeTransportBias
    (freq_cis_target interaction_cis interaction_trans prediction_cis prediction_trans : ℝ) : ℝ :=
  |averagePhaseInteraction freq_cis_target interaction_cis interaction_trans -
    averagePhaseInteraction freq_cis_target prediction_cis prediction_trans|
...
theorem compound_het_not_captured_by_dosage
    (freq_cis interaction_cis interaction_trans prediction_cis prediction_trans : ℝ)
    (h_freq : 0 < freq_cis ∧ freq_cis < 1)
    (h_phase_gap : interaction_cis ≠ interaction_trans)
    (h_pred_cis : prediction_cis = interaction_cis)
    (h_pred_trans : prediction_trans = interaction_trans) :
    haplotypePhasePredictionError freq_cis interaction_cis interaction_trans prediction_cis prediction_trans <
  ...
  have h_zero : haplotypePhasePredictionError freq_cis interaction_cis interaction_trans prediction_cis prediction_trans = 0 := by
...
theorem haplotype_pgs_more_portable_for_cis
    (freq_cis_source freq_cis_target interaction_cis interaction_trans prediction_cis prediction_trans : ℝ)
    (h_freq_shift : freq_cis_source ≠ freq_cis_target)
    (h_phase_gap : interaction_cis ≠ interaction_trans)
    (h_pred_cis : prediction_cis = interaction_cis)
    (h_pred_trans : prediction_trans = interaction_trans) :
    haplotypeTransportBias freq_cis_target interaction_cis interaction_trans prediction_cis prediction_trans <
      dosageTransportBias freq_cis_source freq_cis_target interaction_cis interaction_trans := by
  rw [dosageTransportBias_eq]
  have h_zero : haplotypeTransportBias freq_cis_target interaction_cis interaction_trans prediction_cis prediction_trans = 0 := by
    rw [h_pred_cis, h_pred_trans]
    unfold haplotypeTransportBias averagePhaseInteraction
    have h_sub : freq_cis_target * interaction_cis + (1 - freq_cis_target) * interaction_trans -
