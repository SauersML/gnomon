...
theorem compound_het_not_captured_by_dosage
  ...
  have h_zero : freq_cis * (interaction_cis - interaction_cis) ^ 2 +
      (1 - freq_cis) * (interaction_trans - interaction_trans) ^ 2 = 0 := by ring
  rw [h_zero]
...
theorem haplotype_pgs_more_portable_for_cis
  ...
  have h_zero : |averagePhaseInteraction freq_cis_target interaction_cis interaction_trans -
      averagePhaseInteraction freq_cis_target interaction_cis interaction_trans| = 0 := by rw [h_inner, abs_zero]
  rw [h_zero]
  exact mul_pos
