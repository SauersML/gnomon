...
noncomputable def haplotypePhasePredictionError
    (freq_cis interaction_cis interaction_trans hap_cis_pred hap_trans_pred : ℝ) : ℝ :=
  freq_cis * (interaction_cis - hap_cis_pred) ^ 2 +
    (1 - freq_cis) * (interaction_trans - hap_trans_pred) ^ 2
...
noncomputable def haplotypeTransportBias
    (freq_cis_target interaction_cis interaction_trans hap_cis_pred hap_trans_pred : ℝ) : ℝ :=
  |averagePhaseInteraction freq_cis_target interaction_cis interaction_trans -
    averagePhaseInteraction freq_cis_target hap_cis_pred hap_trans_pred|
...
theorem compound_het_not_captured_by_dosage
    (freq_cis interaction_cis interaction_trans hap_cis_pred hap_trans_pred : ℝ)
    (h_cis : hap_cis_pred = interaction_cis)
    (h_trans : hap_trans_pred = interaction_trans)
    (h_freq : 0 < freq_cis ∧ freq_cis < 1)
    (h_phase_gap : interaction_cis ≠ interaction_trans) :
    haplotypePhasePredictionError freq_cis interaction_cis interaction_trans hap_cis_pred hap_trans_pred <
  ...
  have h_hap_zero : haplotypePhasePredictionError freq_cis interaction_cis interaction_trans hap_cis_pred hap_trans_pred = 0 := by
    rw [h_cis, h_trans]
...
theorem haplotype_pgs_more_portable_for_cis
    (freq_cis_source freq_cis_target interaction_cis interaction_trans hap_cis_pred hap_trans_pred : ℝ)
    (h_cis : hap_cis_pred = interaction_cis)
    (h_trans : hap_trans_pred = interaction_trans)
    (h_freq_shift : freq_cis_source ≠ freq_cis_target)
    (h_phase_gap : interaction_cis ≠ interaction_trans) :
    haplotypeTransportBias freq_cis_target interaction_cis interaction_trans hap_cis_pred hap_trans_pred <
      dosageTransportBias freq_cis_source freq_cis_target interaction_cis interaction_trans := by
  rw [dosageTransportBias_eq]
  have h_hap_zero : haplotypeTransportBias freq_cis_target interaction_cis interaction_trans hap_cis_pred hap_trans_pred = 0 := by
    rw [h_cis, h_trans]
    unfold haplotypeTransportBias
    have h_eq : averagePhaseInteraction freq_cis_target interaction_cis interaction_trans - averagePhaseInteraction freq_cis_target interaction_cis interaction_trans = 0 := sub_self _
    rw [h_eq]
    exact abs_zero
  rw [h_hap_zero]
  exact mul_pos
