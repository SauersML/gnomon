...
structural phase-misspecification error when predictions are exact. -/
...
noncomputable def haplotypeTransportBias
    (freq_cis_target interaction_cis_target interaction_trans_target pred_cis pred_trans : ℝ) : ℝ :=
  freq_cis_target * |interaction_cis_target - pred_cis| +
    (1 - freq_cis_target) * |interaction_trans_target - pred_trans|
