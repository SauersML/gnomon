...
allows distinct predictions for each state, leaving a structural error that
depends on its specific estimates. -/
...
/-- Transport bias for a phase-aware haplotype model is the difference
between the true target average effect and the predicted target average
effect using the model's phase-specific estimates. -/
