...
noncomputable def haplotypeTransportBias
  ...
  |freq_cis_target * (interaction_cis - interaction_cis)| +
    |freq_cis_source * (interaction_trans - interaction_trans)|
...
theorem haplotypeTransportBias_eq_zero
  ...
  have hc : interaction_cis - interaction_cis = 0 := sub_self _
  have ht : interaction_trans - interaction_trans = 0 := sub_self _
  rw [hc, ht]
  have hz1 : |freq_cis_target * 0| = 0 := by rw [mul_zero, abs_zero]
  have hz2 : |freq_cis_source * 0| = 0 := by rw [mul_zero, abs_zero]
  rw [hz1, hz2, add_zero]
