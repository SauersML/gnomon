...
noncomputable def haplotypePhasePredictionError
    (freq_cis interaction_cis pred_interaction_cis interaction_trans pred_interaction_trans : ℝ) : ℝ :=
  freq_cis * (interaction_cis - pred_interaction_cis) ^ 2 +
    (1 - freq_cis) * (interaction_trans - pred_interaction_trans) ^ 2
...
noncomputable def haplotypeTransportBias
    (freq_cis_source freq_cis_target interaction_cis interaction_trans pred_interaction_cis pred_interaction_trans : ℝ) : ℝ :=
  |(freq_cis_target * (interaction_cis - pred_interaction_cis) + (1 - freq_cis_target) * (interaction_trans - pred_interaction_trans)) -
    (freq_cis_source * (interaction_cis - pred_interaction_cis) + (1 - freq_cis_source) * (interaction_trans - pred_interaction_trans))|
...
theorem compound_het_not_captured_by_dosage
    ...
    haplotypePhasePredictionError freq_cis interaction_cis interaction_cis interaction_trans interaction_trans < dosagePhaseMisspecificationError freq_cis interaction_cis interaction_trans := by
  rcases h_freq with ⟨h_freq_pos, h_freq_lt_one⟩
  have h_zero : haplotypePhasePredictionError freq_cis interaction_cis interaction_cis interaction_trans interaction_trans = 0 := by
    unfold haplotypePhasePredictionError; ring
  rw [dosagePhaseMisspecificationError_eq, h_zero]
...
theorem haplotype_pgs_more_portable_for_cis
    ...
    (h_phase_gap : interaction_cis ≠ interaction_trans) :
    haplotypeTransportBias freq_cis_source freq_cis_target interaction_cis interaction_trans interaction_cis interaction_trans < dosageTransportBias
      freq_cis_source freq_cis_target interaction_cis interaction_trans := by
  have h_zero : haplotypeTransportBias freq_cis_source freq_cis_target interaction_cis interaction_trans interaction_cis interaction_trans = 0 := by
    unfold haplotypeTransportBias; ring_nf; exact abs_zero
  rw [dosageTransportBias_eq, h_zero]
  exact mul_pos
