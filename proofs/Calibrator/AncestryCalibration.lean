...
/-- **Recalibrated R².**
    Proportion of variance explained after optimal recalibration,
    given by the source R² multiplied by the squared effect correlation. -/
noncomputable def recalibratedR2 (r2_source ρ_sq : ℝ) : ℝ :=
  r2_source * ρ_sq

/-- **Turnover Loss R².**
    Non-recoverable R² loss due to effect turnover (changes in causal variants). -/
noncomputable def turnoverLossR2 (r2_source ρ_sq : ℝ) : ℝ :=
  r2_source * (1 - ρ_sq)
...
/-- **Recalibration recovers R² up to effect turnover limit.**
    ...
    The total source R² decomposes perfectly into the recalibrated R²
    and the turnover loss. -/
theorem recalibration_recovers_up_to_turnover
    (r2_source ρ_sq : ℝ)
    (h_ρ : 0 ≤ ρ_sq) (h_ρ_le : ρ_sq ≤ 1)
    (h_r2 : 0 < r2_source) :
    recalibratedR2 r2_source ρ_sq + turnoverLossR2 r2_source ρ_sq = r2_source := by
  unfold recalibratedR2 turnoverLossR2
  ring
