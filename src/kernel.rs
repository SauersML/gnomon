...
///
/// This function is now allocation-free. It uses the pre-validated buffer
/// provided in `KernelInput` for all its calculations.
#[inline]
pub fn accumulate_scores_for_person(mut input: KernelInput) {
    ...
    let mut idx_g1 = Vec::with_capacity(input.weights.num_snps / 4);
    let mut idx_g2 = Vec::with_capacity(input.weights.num_snps / 16);
    for (snp_idx, &dosage) in input.genotype_row.iter().enumerate() {
        match dosage {
            1 => idx_g1.push(snp_idx),
            2 => idx_g2.push(snp_idx),
            _ => (),
        }
    }
