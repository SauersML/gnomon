...
REALISTIC_DIMENSIONS = [
    {
        ...
        # Purpose: The core "grudge match" on the full cohort.
        "n_individuals": 2_000,
        ...
        # No "subset_pct" key means this runs on 100% of individuals.
    },
    {
        "test_name": "WGS_Standard_Rare_SUBSET",
        # Purpose: Standard WGS simulation on a subset.
        "n_individuals": 1_000,
        "genome_variants": 500_000,
        ...
        "subset_pct": 0.10,  # Run on a 10% random subset.
    },
    {
        "test_name": "Multi_Panel_Modest_SUBSET",
        # Purpose: Multi-panel merge test on a subset.
        "n_individuals": 1_500,
        ...
        "subset_pct": 0.05,  # Run on a 5% random subset.
    },
    {
        "test_name": "LargeN_ModestK_ACAF_SUBSET",
        # Purpose: Large cohort test on a small subset.
        "n_individuals": 5_000,
        ...
        "subset_pct": 0.01,  # Run on a 1% random subset.
    },
]
...
class RealisticDataGenerator:
    ...
    def _generate_bed(self):
        ...
        with open(self.run_prefix.with_suffix(".bed"), 'wb') as f:
            ...
            for i in range(n_variants):
                ...
                codes = mapping_array[genotypes + 1] # Adjust for mapping_array index
    ...
    def _generate_score_files(self) -> List[Path]:
        ...
        for sf_config in self.params['score_files']:
            ...
            for i in range(sf_config['n_scores']):
                ...
                missing_weight_pct = sf_config.get('missing_weight_pct', 0.0)
                if missing_weight_pct > 0 and n_weighted > 0:
                    n_missing = int(missing_weight_pct * n_weighted)
    ...
    def cleanup(self):
        ...
        for f in self.workdir.glob("plink2_run*"): f.unlink(missing_ok=True)
        multi_score_dir = self.workdir / f"scores_{self.params['test_name']}"
        if multi_score_dir.is_dir(): shutil.rmtree(multi_score_dir)
...
def run_and_monitor_process(tool_name: str, command: List[str], cwd: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        while process.poll() is None:
            try: peak_rss_mb = max(peak_rss_mb, p.memory_info().rss / 1024 / 1024)
        ...
        return {"tool": tool_name, "time_sec": -1, "peak_mem_mb": -1, "success": False}
...
def report_results(results: list):
    ...
    try:
        ...
        if not success_df.empty:
            pivoted = success_df.pivot_table(index='test_name', columns='tool', values=['time_sec', 'peak_mem_mb'], aggfunc='first')
    ...
    for run_id, workload_params in enumerate(REALISTIC_DIMENSIONS, 1):
        ...
        try:
            ...
            if subset_pct and subset_pct < 1.0:
                ...
                fam_df.sample(n=sample_size, random_state=run_id).to_csv(keep_file_path, sep='\t', header=False, index=False)
            ...
            # --- PLINK2 Execution (Unchanged logic, just added --keep) ---
            if gnomon_res["success"]:
                ...
                if len(moved_score_files) > 1:
                    ...
                    for sf_path in moved_score_files:
                        ...
                        if merged_df is None: merged_df = current_df
                        else: merged_df = pd.merge(merged_df, current_df, on=id_cols, how='outer')
                    
                    score_columns = [col for col in merged_df.columns if col.startswith('score_')]
                    if score_columns: merged_df[score_columns] = merged_df[score_columns].fillna(0)
            ...
            if plink_merged_score_file and plink_merged_score_file.exists(): plink_merged_score_file.unlink()
