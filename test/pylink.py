...
def generate_variants_and_weights(precise: bool = False):
    """
    FIRST: Derives normally and randomly distributed weights over 50K positions.
    Switches to float128 for weights if 'precise' is True.
    """
    print(f"Step 1: Simulating {N_VARIANTS} variants on chr{CHR}...")

    # Determine the float type based on the precision flag
    float_dtype = np.float128 if precise else np.float64
    if precise:
        print("  > Running in HIGH PRECISION mode (numpy.float128).")
    ...
    effect_weights = np.random.normal(loc=0.5, scale=0.5, size=N_VARIANTS).astype(float_dtype)
...
def calculate_ground_truth_prs(genotypes, variants_df, precise: bool = False):
    """
    THEN: Calculates the 'ground truth' polygenic score for each individual.
    Uses high-precision float128 for calculations if 'precise' is True.
    """
    ...
    for i in range(n_individuals):
        ...
        valid_weights = variants_df['effect_weight'][valid_mask].values.astype(float_dtype)
        dosages = np.zeros_like(valid_genotypes, dtype=float_dtype)
...
def write_output_files(prs_results, variants_df, genotypes_with_missing, prefix: Path, precise: bool = False):
    """
    Writes all specified output files to the workspace directory.
    Adjusts float formatting for the truth file based on the 'precise' flag.
    """
    ...
    # Use full precision for output if the flag is set, otherwise use a standard format
    float_format = None if precise else '%.6g'
    if precise:
        print(f"  > Writing truth .sscore with FULL precision to {sscore_filename}")
    else:
        print(f"  > Writing truth .sscore with standard precision to {sscore_filename}")

    with open(sscore_filename, 'w') as f:
        f.write('#' + '\t'.join(prs_results_ordered.columns) + '\n')
        prs_results_ordered.to_csv(f, sep='\t', header=False, index=False, na_rep='NA', float_format=float_format)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run a full simulation and validation pipeline for PRS scoring tools.")
    parser.add_argument(
        '--precise',
        action='store_true',
        help='Switch ground truth calculation and output to maximum precision (numpy.float128).'
    )
    parser.add_argument(
        '--keep-files',
        action='store_true',
        help='Do not clean up the workspace directory upon completion.'
    )
    ...
    try:
        ...
        variants_df = generate_variants_and_weights(precise=args.precise)
        genotypes_pristine = generate_genotypes(variants_df)
        genotypes_with_missing = introduce_missingness(genotypes_pristine)
        prs_results_df = calculate_ground_truth_prs(genotypes_with_missing, variants_df, precise=args.precise)
        write_output_files(prs_results_df, variants_df, genotypes_with_missing, OUTPUT_PREFIX, precise=args.precise)
        ...
        if not args.keep_files:
            ...
            print("\nSkipping cleanup because '--keep-files' argument was provided.")
