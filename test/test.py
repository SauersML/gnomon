...
def validate_results(pgs_id: str, results: list):
    """Loads successful tool outputs and calculates correlation and differences."""
    ...
    for res in results:
        if res['success']:
            ...
            if tool_name == 'gnomon':
                # Assuming gnomon output path needs to be constructed
                successful_tools[tool_name] = CI_WORKDIR / f"{tool_name}_{pgs_id}.sscore"
            elif tool_name == 'plink1':
                successful_tools[tool_name] = CI_WORKDIR / f"{tool_name}_{pgs_id}.profile"
    ...
    for tool, path in successful_tools.items():
        ...
        if tool == 'plink1':
            ...
            df = df[['IID', 'SCORE']].rename(columns={'SCORE': f'SCORE_{tool}'})
        else: # gnomon and plink2
             # This assumes gnomon output is similar to plink2 .sscore format
            df = pd.read_csv(path, sep='\s+')
            score_col = next((col for col in df.columns if 'SCORE' in col), None)
            if score_col:
                df = df[['#IID', score_col]].rename(columns={'#IID': 'IID', score_col: f'SCORE_{tool}'})
            else:
                print(f"  > WARNING: Could not find a SCORE column in {path}", flush=True)
                continue

        if merged_df is None:
            merged_df = df
        else:
            merged_df = pd.merge(merged_df, df, on='IID')
