...
def main():
    ...
    if conclusion == "success":
        prompt = (
            ...
            "(specifically files in proofs/). Do not create new files. Do not edit any file besides 'proofs/*.lean'. You must successfully compile the code yourself. If the build times out or fails, do not submit it and keep working. You are not allowed to use 'native_decide' or similar."
            "If the build executes and terminates the shell, count that as a failure. Always tail build logs. You can optimize code, strengthen proofs, and replace axioms with actual proofs. Feel free to try big or multiple tasks. We should also remove and fix specification gaming or vacuous verification. It involves writing theorem statements that appear rigorous in natural language but are mathematically constructed to be trivial or tautological. The most common tactic is begging the question, where the theorem explicitly includes the desired conclusion within its own hypothesis, rendering the proof a simple restatement of the input. Another tactic is the trivial witness, where a property regarding a complex mathematical object is proven by providing a hardcoded constant that technically satisfies a loose inequality without actually computing or representing the complex object itself. Finally, ex post facto construction involves defining a bounding function or rule only after calculating the specific error value, ensuring the condition is met by definition rather than by deriving a meaningful general law. For all of these, if they occur, we need to address them well and improve the code."
            "IMPORTANT: Prefer a visible `sorry` to a custom axiom, premise laundering, vacuity, a weakened target, or an imported result that merely supplies the answer. A declaration containing `sorry` is incomplete and must never be described as proved or trusted. Keep trying for a real proof when the statement is sound, but do not manufacture one by changing its meaning. Do not assume more than is necessary. Do not attempt low-importance small changes like style improvements, comments, etc."
